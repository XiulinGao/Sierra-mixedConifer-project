...
invntry_dates = list(
  ...
  STEF = c(2009, 2012, 2014, 2016, 2018),
...
if(site=='Teakettle'){
  ...
  if(site=='Blodgett'){
    ...
    avg_thin <- avg_ba_by_unit[(avg_ba_by_unit$Treatment=='Thin' | 
                                 avg_ba_by_unit$Treatment == 'None') &
                                 avg_ba_by_unit$Start_yr!=ref_year,] %>%
      group_by(Unit, Treatment) %>%
      summarise(Time_wghtd_avg = weighted.mean(var, nYr))
    
    avg_burn <- avg_ba_by_unit[avg_ba_by_unit$Start_yr>2013 & 
                                  (avg_ba_by_unit$Treatment=='Burn' | 
                                 avg_ba_by_unit$Treatment == 'Burn+Thin'),] %>%
      group_by(Unit, Treatment) %>%
      summarise(Time_wghtd_avg = weighted.mean(var, nYr))
...
for(trt in c('Burn', 'Thin', 'Burn+Thin', 'None')){
  print(trt)
...
## Rescale composition excluding all species except cedar, fir, pine, oak
subset_pft <- pft_by_unit %>% 
  filter(PFT %in% c('cedar', 'fir', 'pine', 'oak')) 

if(site == 'Teakettle'){
  pft_fracs <- subset_pft %>% 
  ...
  study_avg <- all_pft_avgs %>% 
   ...
   if(site=='Blodgett'){
      ...
      avg_thin <- all_pft_avgs[(all_pft_avgs$Treatment=='Thin' | 
                                 all_pft_avgs$Treatment == 'None') &
                                 all_pft_avgs$Start_yr!=ref_year,] %>%
        group_by(Unit, Treatment, PFT) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
      
      avg_burn <- all_pft_avgs[all_pft_avgs$Start_yr>2013 & 
                                    (all_pft_avgs$Treatment=='Burn' |
                                       all_pft_avgs$Treatment=='Burn+Thin'),] %>%
        group_by(Unit, Treatment, PFT) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
...
prepost_by_pft <- data.frame()
  ...
  for(p in c('cedar', 'fir', 'pine', 'oak')){
    if(site %in% c('Blodgett', 'STEF')){
      trt_lst <- c('Burn', 'Thin', 'Burn+Thin', 'None')
    }else if(site == 'Teakettle'){
      trt_lst <- c('Burn', 'Thin', 'Burn+Thin')
    }
    
    for(trt in trt_lst){
      print(paste(p, trt, sep = ' '))
...
pft_time_plot <- ggplot(pft_means_all_years,
              aes(x=xlbl, y=emmean, ymin=emmean-SE, ymax=emmean+SE, ## ribbons represent standard error; can replace with confidence interval if desired
                  fill=PFT))+
  ...
  if(site=='Blodgett'){
    study_avg <- avg_tph_by_unit[avg_tph_by_unit$Start_yr!=ref_year,] %>%
      ...
      avg_thin <- avg_tph_by_unit[(avg_tph_by_unit$Treatment=='Thin' | 
                                     avg_tph_by_unit$Treatment == 'None') &
                                    avg_tph_by_unit$Start_yr!=ref_year,] %>%
        group_by(Unit, Treatment) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
      
      avg_burn <- avg_tph_by_unit[avg_tph_by_unit$Start_yr>2013 & 
                                    (avg_tph_by_unit$Treatment=='Burn' |
                                       avg_tph_by_unit$Treatment == 'Burn+Thin'),] %>%
        group_by(Unit, Treatment) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
...
tph_time_plot <- ggplot(tph_means_all_years,
              aes(x=xlbl, y=emmean, ymin=emmean-SE, ymax=emmean+SE, ## ribbons represent standard error; can replace with confidence interval if desired
                  col=Treatment,fill=Treatment))+
  geom_bar(stat="identity", position = 'dodge') +
    geom_errorbar(position = 'dodge') +
...
if(site == 'Teakettle'){
  study_avg <- all_size_avgs %>% 
   ...
   if(site=='Blodgett'){
      ...
      avg_thin <- all_size_avgs[(all_size_avgs$Treatment=='Thin' | 
                                   all_size_avgs$Treatment=='None') & 
                                  all_size_avgs$Start_yr!=ref_year,] %>%
        group_by(Unit, Treatment, Size_class) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
      
      avg_burn <- all_size_avgs[all_size_avgs$Start_yr>2013 & 
                                    (all_size_avgs$Treatment=='Burn' | 
                                       all_size_avgs$Treatment=='Burn+Thin'),] %>%
        group_by(Unit, Treatment, Size_class) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
...
growth_params <- read.csv(file.path('..', 'data', 'Benchmarking_data', 
                                    'Growth_all_sites', 'params_by_site_dbh_2026-01-21.csv'))

## Adjust data nomenclature
if(site=='Blodgett'){
  site_df <- growth_params[growth_params$Site=='Blodgett',]
}else if(site == 'STEF'){
...
print(growth_trt_diffs + facet_wrap(~DBH, scales = "free_y"))
```
...
if(site=='STEF'){
  out[out$Treatment %in% c('Burn', 'Burn+Thin') & 
        out$start_yr<2014 & 
        out$Timeframe!='Post-treatment', 'IncludesPreTreat'] = 'Yes'
  out[out$Treatment %in% c('Thin', 'None') & 
        out$start_yr==2009 & 
        out$Timeframe!='Post-treatment', 'IncludesPreTreat'] = 'Yes'
}else if(site =='Blodgett'){
...
```{r mort_interval_plot}
mort_time_plot <- ggplot(out[out$Timeframe=='Period mortality',],
       aes(x=start_yr, y=prob, 
                               ymin=prob - SE, 
                               ymax=prob + SE, 
                               fill=Treatment
                               ))  + 
...
mort_time_plot <- mort_time_plot+facet_wrap(~Category) #, scales = 'free_y')
...
```{r mort_wPre_plot}
mort_study_plot <- ggplot(out[out$Timeframe=='Study mortality',],
...
print(mort_study_plot)

```

```{r mort_postTrt_plot}

mort_postTrt_plot <- ggplot(out[out$Timeframe=='Post-treatment',],
       aes(x=Category, y=prob, 
                               ymin=prob - SE, 
                               ymax=prob + SE, 
                               fill=Treatment
...
fuel_by_trt <- fuel_by_trt[fuel_by_trt$Fuel_class %in% c('1-hr', '10-hr', '100-hr'),] %>%
  ...
  summarise(
    ...
    ### Get the standard error of the summed class
    ### This is the upper bound on standard error (i.e., 1, 10, 100h estimates are 100% correlated)
    SE = sum(SE)
  ) %>%
  bind_rows(fuel_by_trt, .)
...
fuel_by_trt <- read.csv('../data/Benchmarking_data/Blodgett/fuel_by_treatment_year.csv')


## Add a fuel class for the sum of 1, 10, 100hr fuels 
fuel_by_trt <- fuel_by_trt %>%
  filter(Fuel_class %in% c('1h_Mg_ha', '10h_Mg_ha', '100h_Mg_ha')) %>%
  group_by(Treatment, Year) %>%
  summarise(
    Fuel_load = sum(Fuel_load),
...
## First, get comparison in each year of study
### Initialize dataframes to append yearly results to
fuel_means_trt <- data.frame()

for(fuel in unique(fuel_by_trt$Fuel_class)){
  for(year in unique(fuel_by_trt$Year)){
    # Get means and contrasts for fuel class
    fuel_outs <- calculate_pct_diff(fuel_by_trt[fuel_by_trt$Fuel_class==fuel &
                                                  fuel_by_trt$Year==year,], 
                                    compare_column = 'Treatment', 
                                    ref = 'None', 
                                    out_col = 'Fuel_load')
    # Add a Fuel column
...
write.csv(fuel_means_trt, file.path(paste0('../data/Benchmarking_data/Benchmarks/fuels_', site, '.csv')), row.names=FALSE)
...
fuel_time_plot <- fuel_time_plot+facet_wrap(~Fuel_class, scales = 'free_y', ncol = 2)

print(fuel_time_plot)
