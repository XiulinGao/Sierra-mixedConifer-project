...
avg_ba_by_unit <- compute_avgs_over_intervals(ba_by_unit, var = 'BAperTree_m2ha')
avg_ba_by_unit
...
ba_means_all_years$xlbl <- paste(ba_means_all_years$Start_yr, ba_means_all_years$End_yr, sep = '-')

ba_time_plot <- ggplot(ba_means_all_years[ba_means_all_years$Start_yr!=thin_ref,],
...
ba_trt_diffs <- ggplot(ba_means_all_years[ba_means_all_years$Treatment!='None' & 
                                            ba_means_all_years$Start_yr!=thin_ref,],
...
```{r pft_timeseries}
## Visualize absolute basal area over time and across treatments
pft_means_all_years$xlbl <- paste(pft_means_all_years$Start_yr, pft_means_all_years$End_yr, sep = '-')

pft_time_plot <- ggplot(pft_means_all_years[pft_means_all_years$Start_yr!=thin_ref,],
              aes(x=xlbl, y=emmean, ymin=emmean-SE, ymax=emmean+SE, ## ribbons represent standard error; can replace with confidence interval if desired
                  fill=PFT))+
...
```{r tph_timeseries}
## Visualize absolute basal area over time and across treatments
tph_means_all_years$xlbl <- paste(tph_means_all_years$Start_yr, tph_means_all_years$End_yr, sep = '-')
...
size_means_all_years$xlbl <- paste(size_means_all_years$Start_yr, size_means_all_years$End_yr, sep = '-')
...
for(y in 2:length(invntry_dates[[site]])){
  ...
  if(site=='STEF' & y == 2){
    ...
    m <- make_mrtlty_tbl(raw[raw$DBH_BA >= 15,], status_col = 'Status', 
                       dead_ind = dead_ind, ivl = ivl, 
                       t=invntry_dates[[site]][y]-invntry_dates[[site]][y-1], 
                       rndm_fx = 'Unit')
  }
...
read.csv('../data/Benchmarking_data/Blodgett/mortality_invntry_2001-2003.csv')
