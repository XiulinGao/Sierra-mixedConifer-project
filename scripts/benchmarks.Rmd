...
surv_df <- raw15[!is.na(raw15$start_year),] %>%
  group_by(get(treeid)) %>%
...
out <- fuel_means_trt[, c('Treatment', 'emmean', 'percent_diff', 'Fuel_class', 'Year')]
colnames(out) <- c('Treatment', 'Fuel_biomass', 'Cntrl_pct_diff', 'Fuel_class', 'Year')

write.csv(out, file.path(paste0('../data/Benchmarking_data/Benchmarks/fuels_cntrlTreat_', site, '.csv')), row.names=FALSE)
...
out_pre_post <- fuel_pre_post[fuel_pre_post$Timeframe=='Post', 
                              c('Treatment', 'emmean', 'percent_diff', 'Fuel_class')]
colnames(out_pre_post) <- c('Treatment', 'Fuel_biomass', 'PrePost_pct_diff', 'Fuel_class')
...
write.csv(fuel_means_trt, file.path(paste0('../data/Benchmarking_data/Benchmarks/fuels_cntrlTreat_', site, '.csv')), row.names=FALSE)
