...
invntry_dates = list(
  ...
  Teakettle = c('DBH_Pre', 'DBH_Post', 'DBH_10Post', 'DBH_15Post', 'DBH_1PostRX', 'DBH_2PostRX')
...
if(site=='Blodgett'){
  ...
  # Load 2009 data, which was digitized from Knapp et al. 2017
  stef_pre <- read.csv('../data/Benchmarking_data/STEF/preThin_metrics.csv')
...
calculate_simple_diff <- function(emm_df, compare_column, ref, out_col){
  ref_value <- emm_df[[out_col]][emm_df[[compare_column]] == ref]
  
  ## Add simple difference
  emm_df$diff <- emm_df[[out_col]] - ref_value
  ...
  for (i in 1:nrow(emm_df)) {
    if (emm_df[[compare_column]][i] != ref) {
      ...
      # SE of simple difference
      emm_df$se_diff[i] <- sqrt(se_c^2 + se_t^2)
    }
  }
  return(emm_df)
}
...
if(site=='Teakettle'){
  ...
  if(site=='Blodgett'){
    study_avg <- avg_ba_by_unit[avg_ba_by_unit$Start_yr!=ref_year,] %>%
      group_by(Unit, Treatment) %>%
      summarise(Time_wghtd_avg = weighted.mean(var, nYr))
  }else if(site=='STEF'){
    avg_thin <- avg_ba_by_unit[avg_ba_by_unit$Treatment=='Thin',] %>%
      group_by(Unit, Treatment) %>%
      summarise(Time_wghtd_avg = weighted.mean(var, nYr))
    
    avg_burn <- avg_ba_by_unit[avg_ba_by_unit$Start_yr>2013 & 
                                  avg_ba_by_unit$Treatment=='Burn',] %>%
      group_by(Unit, Treatment) %>%
      summarise(Time_wghtd_avg = weighted.mean(var, nYr))
    
    study_avg <- rbind(avg_thin, avg_burn)
  }
  }
...
if(site=='Teakettle'){
  pre <- ba_by_unit[ba_by_unit$DBH_timeframe=='DBH_Pre',]
}else if(site == 'STEF'){
  pre <- ba_by_unit[ba_by_unit$Treatment == 'Burn' & ba_by_unit$Year==2012,]
}else if(site == 'Blodgett'){
  pre <- ba_by_unit[ba_by_unit$Year==ref_year,]
}

pre$DBH_timeframe <- 'Pre'
colnames(pre)[colnames(pre)=='BAperTree_m2ha'] = 'var'
pre <- pre[, colnames(pre)!='Year']

post <- study_avg
post$DBH_timeframe <- 'Post'
colnames(post)[colnames(post)=='Time_wghtd_avg'] = 'var'

prepost <- rbind(pre, post)

prepost_by_trt <- vector('list', length = 3)

if(site=='STEF'){
  ## Get pre-post differences for burned units
  prepost_burn <- make_emm_df(prepost[prepost$Treatment=='Burn',],
                                    compare_column = 'DBH_timeframe',
                                    var = 'var',
                                    ref = 'Pre',
                                    rndm_fx = 'Unit')
  
  prepost_burn$Treatment <- 'Burn'
  
  ## Get pre-post differences for thinned units
  stef_thin <- prepost[prepost$Treatment=='Thin',]
  mean_thin_post <- mean(stef_thin$var)
  se_thin_post <- sd(stef_thin$var)/sqrt(length(stef_thin$var))
  
  ## Pre
  thin_pre <- stef_pre[stef_pre$Metric=='Basal area' & stef_pre$Treatment!='None',]
  mean_thin_pre <- mean(thin_pre$value)
  se_thin_pre <- sqrt((thin_pre$SE[1]^2 + thin_pre$SE[2]^2)/2)
  
  prepost_thin = data.frame('DBH_timeframe'=c('Pre', 'Post'), "emmean" = c(mean_thin_pre, mean_thin_post), 'SE' = c(se_thin_pre, se_thin_post), "Treatment" = 'Thin')
  prepost_thin = calculate_pct_diff(prepost_thin, 'DBH_timeframe', 'Pre', 'emmean')
  
  prepost_out <- bind_rows(prepost_burn, prepost_thin)
  
}else{
  for(trt in c('Burn', 'Thin', 'Burn+Thin')){
    prepost_benchmarks <- make_emm_df(prepost[prepost$Treatment==trt,], compare_column = 'DBH_timeframe', 
                                    var = 'var', 
                                    ref = 'Pre',
                                    rndm_fx = 'Unit')
    
    prepost_benchmarks$Treatment <- trt
    
    prepost_by_trt[[trt]] = prepost_benchmarks
  
  }
  
  prepost_out <- bind_rows(prepost_by_trt)
}

prepost_out <- clean_emm_tbl(prepost_out, var = 'var', 'PrePost')

write.csv(prepost_out, file.path(paste0('../data/Benchmarking_data/Benchmarks/prepost_basal_area_', site, '.csv')), row.names=FALSE)
...
if(site == 'Teakettle'){
  ...
  pft_fracs <- pft_fracs[!(pft_fracs$DBH_timeframe %in% c('DBH_2008')),]
...
for(p in unique(pft_fracs$PFT)){
  if(site=='Teakettle'){
    p_df <- pft_fracs[pft_fracs$DBH_timeframe!= 'DBH_Pre' & pft_fracs$PFT==p,]
  }else{
...
if(site == 'Teakettle'){
  study_avg <- all_pft_avgs %>% 
   ...
   if(site=='Blodgett'){
      study_avg <- all_pft_avgs[all_pft_avgs$Start_yr!=ref_year,] %>%
        group_by(Unit, Treatment, PFT) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
    }else if(site=='STEF'){
      avg_thin <- all_pft_avgs[all_pft_avgs$Treatment=='Thin',] %>%
        group_by(Unit, Treatment, PFT) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
      
      avg_burn <- all_pft_avgs[all_pft_avgs$Start_yr>2013 & 
                                    all_pft_avgs$Treatment=='Burn',] %>%
        group_by(Unit, Treatment, PFT) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
...
### Compare pre- to post-treatment
if(site=='Teakettle'){
  pre <- pft_fracs[pft_fracs$DBH_timeframe=='DBH_Pre',]
}else if(site == 'STEF'){
  pre <- pft_fracs[pft_fracs$Treatment == 'Burn' & pft_fracs$Year==2012,]
}else if(site == 'Blodgett'){
  pre <- pft_fracs[pft_fracs$Year==ref_year,]
}

pre$DBH_timeframe <- 'Pre'
colnames(pre)[colnames(pre)=='PFT_fraction'] = 'var'
pre <- pre[, colnames(pre)!='Year']
...
prepost_by_pft <- data.frame()

if(site == 'STEF'){
  for(p in c('cedar', 'fir', 'pine', 'oak')){
    prepost_out <- make_emm_df(prepost[prepost$Treatment=='Burn' & prepost$PFT==p,],
                                      compare_column = 'DBH_timeframe',
                                      var = 'var',
                                      ref = 'Pre',
                                      rndm_fx = 'Unit')
    
    prepost_out$Treatment <- 'Burn'
    
    prepost_out$PFT <- p
    
    prepost_by_pft <- rbind(prepost_by_pft, prepost_out)
  }
  }else{
    for(p in c('cedar', 'fir', 'pine', 'oak')){
      for(trt in c('Burn', 'Thin', 'Burn+Thin')){
        if(sum(prepost$var[prepost$Treatment==trt
                           & prepost$PFT==p])!=0){
          prepost_benchmarks <- make_emm_df(prepost[prepost$Treatment==trt
                                                    & prepost$PFT==p,], 
                                            compare_column = 'DBH_timeframe', 
                                            var = 'var', 
                                            ref = 'Pre',
          ...
          prepost_benchmarks$PFT <- p
          
          prepost_by_pft <- rbind(prepost_by_pft, prepost_benchmarks)
        }
    }
    }
  }
...
prepost_out <- clean_emm_tbl(prepost_by_pft, var = 'var', 'PrePost')

write.csv(prepost_out, file.path(paste0('../data/Benchmarking_data/Benchmarks/prepost_pft_composition_', site, '.csv')), row.names=FALSE)
...
if(site == 'Teakettle'){
  ...
  if(site=='Blodgett'){
    study_avg <- avg_tph_by_unit[avg_tph_by_unit$Start_yr!=ref_year,] %>%
      group_by(Unit, Treatment) %>%
      summarise(Time_wghtd_avg = weighted.mean(var, nYr))
    }else if(site=='STEF'){
      avg_thin <- avg_tph_by_unit[avg_tph_by_unit$Treatment=='Thin',] %>%
        group_by(Unit, Treatment) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
      
      avg_burn <- avg_tph_by_unit[avg_tph_by_unit$Start_yr>2013 & 
                                    avg_tph_by_unit$Treatment=='Burn',] %>%
        group_by(Unit, Treatment) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
...
### Compare pre- to post-treatment
if(site=='Teakettle'){
  pre <- tph_by_unit[tph_by_unit$DBH_timeframe=='DBH_Pre',]
}else if(site == 'STEF'){
  pre <- tph_by_unit[tph_by_unit$Treatment == 'Burn' & tph_by_unit$Year==2012,]
}else if(site == 'Blodgett'){
  pre <- tph_by_unit[tph_by_unit$Year==ref_year,]
}

pre$DBH_timeframe <- 'Pre'
colnames(pre)[colnames(pre)=='TPH'] = 'var'
pre <- pre[, colnames(pre)!='Year']
...
if(site=='STEF'){
  ...
  ## Pre
  thin_pre <- stef_pre[stef_pre$Metric=='Stand density' & stef_pre$Treatment!='None',]
  mean_thin_pre <- mean(thin_pre$value)
  se_thin_pre <- sqrt((thin_pre$SE[1]^2 + thin_pre$SE[2]^2)/2)
...
write.csv(prepost_out, file.path(paste0('../data/Benchmarking_data/Benchmarks/prepost_stand_density_', site, '.csv')), row.names=FALSE)
...
if(site == 'Teakettle'){
  sizeDist_by_unit <- sizeDist_by_unit[!sizeDist_by_unit$DBH_timeframe %in% c('DBH_2008'),]
}

## Load data with custom bin sizes
if(site=='STEF'){
  cstm_sizeDist = read.csv('../data/Benchmarking_data/STEF/custom_size_dist_by_unit_year.csv')
  cstm_sizeDist$Size_class <- factor(cstm_sizeDist$Size_class, unique(cstm_sizeDist$Size_class))
  
  all_cust_avgs <- data.frame()
  for(s in unique(cstm_sizeDist$Size_class)){
    cust_df <- cstm_sizeDist[cstm_sizeDist$Size_class==s,]
    
    cust_new <- compute_avgs_over_intervals(cust_df, var = 'TPH')
    
    cust_new$Size_class <- s
    
    all_cust_avgs <- rbind(all_cust_avgs, cust_new)
  }
}
...
for(s in unique(sizeDist_by_unit$Size_class)){
  if(site=='Teakettle'){
    s_df = sizeDist_by_unit[sizeDist_by_unit$Size_class==s & 
                              sizeDist_by_unit$DBH_timeframe!='DBH_Pre',]
  }else{
    s_df <- sizeDist_by_unit[sizeDist_by_unit$Size_class==s,]
  }
...
if(site == 'Teakettle'){
  study_avg <- all_size_avgs %>% 
   ...
   if(site=='Blodgett'){
      study_avg <- all_size_avgs[all_size_avgs$Start_yr!=ref_year,] %>%
        group_by(Unit, Treatment, Size_class) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
    }else if(site=='STEF'){
      avg_thin <- all_cust_avgs[all_cust_avgs$Treatment=='Thin',] %>%
        group_by(Unit, Treatment, Size_class) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
      
      avg_burn <- all_size_avgs[all_size_avgs$Start_yr>2013 & 
                                    all_size_avgs$Treatment=='Burn',] %>%
        group_by(Unit, Treatment, Size_class) %>%
        summarise(Time_wghtd_avg = weighted.mean(var, nYr))
...
get_mean_se <- function(df){
  return(sqrt((df$SE[1]^2 + df$SE[2]^2)/2))
}
...
### Compare pre- to post-treatment
if(site=='Teakettle'){
  pre <- sizeDist_by_unit[sizeDist_by_unit$DBH_timeframe=='DBH_Pre',]
  }else if(site == 'Blodgett'){
  pre <- sizeDist_by_unit[sizeDist_by_unit$Year==ref_year,]
  }else if(site == 'STEF'){
  pre <- sizeDist_by_unit[sizeDist_by_unit$Treatment == 'Burn' & sizeDist_by_unit$Year==2012,]
}
...
prepost_by_size <- data.frame()

if(site == 'STEF'){
  ## Pull in digitized data for 2009
  pre_thin_digitized <- read.csv('../data/Benchmarking_data/STEF/STEF_Thin_digitized.csv')
  pre_thin_digitized <- pre_thin_digitized[pre_thin_digitized$Metric!='Error',]
  
  mean_thin_pre <- pre_thin_digitized %>%
    group_by(DBH_max) %>%
    summarise(emmean = mean(TPH),
              Treatment = 'Thin',
              DBH_timeframe = 'Pre')
  
  mean_thin_pre$SE <- unlist(
    pre_thin_digitized %>% 
      group_by(DBH_max) %>%
      group_map(~ get_mean_se(.x))
    )
  
  colnames(mean_thin_pre)[colnames(mean_thin_pre)=='DBH_max'] = 'Size_class'
    
  
  for(s in unique(prepost[prepost$Treatment=='Burn',]$Size_class)){
    prepost_burn <- make_emm_df(prepost[prepost$Treatment=='Burn' & prepost$Size_class==s,],
                                      compare_column = 'DBH_timeframe',
                                      var = 'var',
                                      ref = 'Pre',
                                      rndm_fx = 'Unit')
    
    prepost_benchmarks <- calculate_simple_diff(prepost_burn,
                                            compare_column = 'DBH_timeframe',
                                            ref = 'Pre',
                                            out_col = 'emmean')
    ...
    prepost_burn$Size_class <- s
    
    prepost_by_size <- rbind(prepost_by_size, prepost_burn)
  }
  
  ## Get pre-post differences for thinned units
  stef_thin <- prepost[prepost$Treatment=='Thin',]
  
  mean_post_thin <- stef_thin %>% 
    group_by(Size_class) %>%
    summarise(SE = get_se(var),
              emmean = mean(var),
              Treatment = 'Thin',
              DBH_timeframe = 'Post')
  
  mean_post_thin$Size_class <- as.numeric(as.character(mean_post_thin$Size_class))
  
  prepost_thin = bind_rows(mean_thin_pre, mean_post_thin)
  
  prepost_thin <- prepost_thin %>% 
    group_by(Size_class) %>% 
    group_modify(~ calculate_pct_diff(.x, 'DBH_timeframe', 'Pre', 'emmean'))
  
  prepost_thin <- clean_emm_tbl(prepost_thin, var = 'var', prfx = 'PrePost')

  }else{
    for(s in unique(prepost$Size_class)){
      for(trt in c('Burn', 'Thin', 'Burn+Thin')){
        if(sum(prepost$var[prepost$Treatment==trt
                           & prepost$Size_class==s])!=0){
          prepost_benchmarks <- make_emm_df(prepost[prepost$Treatment==trt
                                                    & prepost$Size_class==s,], 
                                            compare_column = 'DBH_timeframe', 
                                            var = 'var', 
                                            ref = 'Pre',
                                            rndm_fx = 'Unit')
          
          prepost_benchmarks <- calculate_simple_diff(prepost_benchmarks,
                                                      compare_column = 'DBH_timeframe',
                                                      ref = 'Pre',
                                                      out_col = 'emmean')
          ...
          prepost_benchmarks$Size_class <- s
          
          prepost_by_size <- rbind(prepost_by_size, prepost_benchmarks)
        }
    }
    }
  }
...
prepost_out <- clean_emm_tbl(prepost_by_size, var = 'var', 'PrePost')

if(site == 'STEF'){
  prepost_out$Size_class <- as.numeric(as.character(prepost_out$Size_class))
  prepost_out <- bind_rows(prepost_out, prepost_thin)
}

write.csv(prepost_out, file.path(paste0('../data/Benchmarking_data/Benchmarks/prepost_size_dist_', site, '.csv')), row.names=FALSE)
...
growth_params <- read.csv(file.path('..', 'data', 'Benchmarking_data', 
                                    'Growth_all_sites', 'params_by_site_dbh_2025-07-08.csv'))

## Adjust data nomenclature
if(site=='Blodgett'){
  site_df <- growth_params[growth_params$Site=='Blodgett_FFS15',]
}else if(site == 'STEF'){
  site_df <- growth_params[growth_params$Site=='STEF-VDT15',]
}else if(site=='Teakettle'){
  site_df <- growth_params[growth_params$Site == 'Teakettle15',]
}

site_growth <- site_df[site_df$Treatment=='None',]

## Note that the only uncertainty metric for growth is the 95% credible interval. 
growth_by_pft_size <- data.frame(PFT = site_growth$PFT, DBH = site_growth$DBH, Mean_cm = exp(site_growth$Mean), low95CI = exp(site_growth$low_ci), high95CI = exp(site_growth$upper_ci))
...
trt_fx <- site_df[site_df$Treatment!='None',]

trtmnt_effect_growth_pft <- data.frame(PFT = trt_fx$PFT, 
                                       DBH = trt_fx$DBH,
                                       Treatment = trt_fx$Treatment, 
                                       Multiplier_mean = exp(trt_fx$Mean),
                                       Multiplier_low95CI = exp(trt_fx$low_ci),
                                       Multiplier_high95CI = exp(trt_fx$upper_ci)
)
...
print(growth_trt_diffs + facet_wrap(~DBH))
```
...
multiply_se <- function(x, y, se_x, se_y){
  return(sqrt(x^2 * se_y^2 + y^2 * se_x^2))
}

get_cumul_SE <- function(temp){
  SE1 <- temp$SE[1]
  surv1 <- temp$survival[1]
  
  for(i in 2:nrow(temp)){
    SE1 <- multiply_se(x = surv1,
                y = temp$survival[i],
                se_x = SE1,
                se_y = temp$SE[i])
    
    surv1 <- surv1 * temp$survival[i]
    
  }
  return(SE1)
}

get_cumul_mort <- function(df, group_cols){
  out <- df %>%
    group_by(across({{group_cols}})) %>% 
    summarise(
    survival = prod(survival),
    prob = 1- prod(survival),
  ...
  out$SE <- unlist(
    df %>% 
      group_by(across({{group_cols}})) %>%
      group_map(~ get_cumul_SE(.x))
    )
  
  if(length(group_cols)==1){
    out <- calculate_simple_diff(out,
                                compare_column = 'Treatment',
                                ref = 'None',
                                out_col = 'prob')  
    
    out <- calculate_pct_diff(out,
                                compare_column = 'Treatment',
                                ref = 'None',
                                out_col = 'prob')  
  }
...
if(site == 'Teakettle'){
  teakettle_unit_dates <- ba_by_unit %>% 
...
get_unit_years <- function(df, t1, t2){
  ## Add the first and last year of the measurement period of the plot
  df$Mort_y1 <- NA
  df$Mort_y2 <- NA

  for(u in unique(df$Plot)){
    df$Mort_y1[df$Plot==u] = teakettle_unit_dates$Year[teakettle_unit_dates$Unit==u &
                                                         teakettle_unit_dates$DBH_timeframe==t1]
    df$Mort_y2[df$Plot==u] = teakettle_unit_dates$Year[teakettle_unit_dates$Unit==u &
                                                         teakettle_unit_dates$DBH_timeframe==t2]
  }
...
coxme_model <- coxme(
  Surv(entry, exit, status) ~ treatment + (1 | unit_id),
...
if(site=='Teakettle'){
  ...
  pfts_in_mort <- c('cedar', 'fir', 'pine')
    ...
    unitid = 'comp'
    year_col = 'year' 
    pfts_in_mort <- c('cedar', 'fir', 'pine', 'oak')
    }else if(site =='STEF'){
      status_col = 'Status'
      dead_ind = 'D'
      rndm_fx = 'Unit'
...
for(y in 2:length(invntry_dates[[site]])){
  ...
  if(site=='Teakettle'){
    ...
    raw <- get_unit_years(raw, t1 = t1, t2 = t2)
    
    m <- make_mrtlty_tbl(raw[raw$DBH_BA >= 15,], status_col=status_col, 
                     dead_ind = dead_ind, 
                     rndm_fx = rndm_fx)
    
    for(trt in unique(m$Treatment)){
      m$start_yr[m$Treatment==trt] <- mean(raw$Mort_y1[raw$Treatment==trt])
      m$end_yr[m$Treatment==trt] <- mean(raw$Mort_y2[raw$Treatment==trt])
    }
    }else{
      m <- make_mrtlty_tbl(raw[raw$DBH_BA >= 15,], status_col=status_col, 
                     dead_ind = dead_ind, 
                     rndm_fx = rndm_fx)
      m <- calculate_simple_diff(m, 
                                   compare_column = 'Treatment',
                                   ref = 'None',
                                   out_col = 'prob')
      m <- calculate_pct_diff(m, 
                                compare_column = 'Treatment',
                                ref = 'None',
                                out_col = 'prob')
      
      m$start_yr <- invntry_dates[[site]][y-1]
      m$end_yr <- invntry_dates[[site]][y]
  }
  
  allmort <- bind_rows(allmort, m)
  
  }
    
allmort <- allmort[,!(colnames(allmort) %in% c('df', 'asymp.LCL', 'asymp.UCL'))]
allmort$Timeframe <- 'Period mortality'

### Get cumulative mortality for entire period
allmort$survival = 1 - allmort$prob

study_mort <- get_cumul_mort(allmort, 
                             group_cols = c('Treatment'))
study_mort$Timeframe <- 'Study mortality'

if(site=='Blodgett'){
  postTrt_mort <- get_cumul_mort(allmort[allmort$start_yr>trt_dates[[site]][['Thin']][[1]] & 
                       allmort$end_yr < trt_dates[[site]][['Thin']][[2]] & 
                         allmort$Treatment %in% c('Thin', 'None'),],
                       group_cols = c('Treatment'))
  postTrt_mort$Timeframe <- 'Post-treatment'
  
  out <- rbind(allmort, study_mort, postTrt_mort)

}else if(site == 'Teakettle'){
  postTrt_mort <- get_cumul_mort(allmort[allmort$start_yr>trt_dates[[site]][['Thin']][[1]],],
                                         group_cols = c('Treatment'))
  postTrt_mort$Timeframe <- 'Post-treatment'
  out <- bind_rows(allmort, study_mort, postTrt_mort)
}else{
  out <- rbind(allmort, study_mort)
}

out$Category <- 'All trees'
...
for(y in 2:length(invntry_dates[[site]])){
    ...
    for(p in pfts_in_mort){
      print(p)
      
      # print(raw[raw$DBH_BA >= 15 & raw$PFT_simple==p & raw$status==dead_ind,] %>% count(comp))
      # print(raw[raw$DBH_BA >= 15 & raw$PFT_simple==p & raw$status==dead_ind,] %>% count(Treatment))
      ...
      if(site=='Teakettle'){
        ...
        m <- make_mrtlty_tbl(raw[raw$DBH_BA >= 15 & raw$PFT_simple==p,], status_col=status_col, 
                         dead_ind = dead_ind, 
                         rndm_fx = rndm_fx)
...
pftmort <- pftmort[,!(colnames(pftmort) %in% c('df', 'asymp.LCL', 'asymp.UCL'))]
pftmort$Timeframe <- 'Period mortality'

### Get cumulative mortality for entire period
pftmort$survival = 1 - pftmort$prob

pftstudy_mort <- get_cumul_mort(pftmort, 
                             group_cols = c('Treatment', 'Category'))

if(site=='Blodgett'){
  postTrt_pftmort <- get_cumul_mort(pftmort[pftmort$start_yr>trt_dates[[site]][['Thin']][[1]] & 
                       pftmort$end_yr < trt_dates[[site]][['Thin']][[2]] & 
                         pftmort$Treatment %in% c('Thin', 'None'),],
                       group_cols = c('Treatment', 'Category'))
  postTrt_pft_diffs <- vector('list', length = length(pfts_in_mort))
...
pft_diffs <- vector('list', length = 4)

for(p in pfts_in_mort){
  temp <- calculate_simple_diff(
    pftstudy_mort[pftstudy_mort$Category==p,], 
    compare_column = 'Treatment',
    ref = 'None',
    out_col = 'prob'
  )
  
  pft_diffs[[p]] <- calculate_pct_diff(
    temp,
    compare_column = 'Treatment',
    ref = 'None',
    out_col = 'prob'
  ...
  if(site!='STEF'){
    temp2 <- calculate_simple_diff(
      postTrt_pftmort[postTrt_pftmort$Category==p,], 
      compare_column = 'Treatment',
      ref = 'None',
      out_col = 'prob'
    )
    
    postTrt_pft_diffs[[p]] <- calculate_pct_diff(
      temp2,
      compare_column = 'Treatment',
      ref = 'None',
      out_col = 'prob'
...
pftstudy_mort <- bind_rows(pft_diffs)
pftstudy_mort$Timeframe <- 'Study mortality'

if(site!='STEF'){
  postTrt_pftmort <- bind_rows(postTrt_pft_diffs)
  postTrt_pftmort$Timeframe <- 'Post-treatment'
  
  out <- bind_rows(out, pftmort, pftstudy_mort, postTrt_pftmort)
}else{
  out <- rbind(out, pftmort, pftstudy_mort)
}
```

```{r mort_small}
smallmort <- data.frame()
size_thresh <- 60
cat_nm <- paste0('<', size_thresh, 'cm')
...
for(y in 2:length(invntry_dates[[site]])){
    ...
    if(site=='Teakettle'){
      ...
      m <- make_mrtlty_tbl(raw[raw$DBH_BA >= 15 & raw$DBH_BA<size_thresh,], status_col=status_col, 
                       dead_ind = dead_ind, 
                       rndm_fx = rndm_fx)
    ...
    smallmort <- rbind(smallmort, m)
  }

smallmort <- smallmort[,!(colnames(smallmort) %in% c('df', 'asymp.LCL', 'asymp.UCL'))]
smallmort$Timeframe <- 'Period mortality'

### Get cumulative mortality for entire period
smallmort$survival = 1 - smallmort$prob

study_smmort <- get_cumul_mort(smallmort, 
                             group_cols = c('Treatment'))
study_smmort$Timeframe <- 'Study mortality'
study_smmort$Category <- cat_nm

if(site=='Blodgett'){
  postTrt_smmort <- get_cumul_mort(smallmort[smallmort$start_yr>trt_dates[[site]][['Thin']][[1]] & 
                       smallmort$end_yr < trt_dates[[site]][['Thin']][[2]] & 
                         smallmort$Treatment %in% c('Thin', 'None'),],
                       group_cols = c('Treatment'))
  postTrt_smmort$Timeframe <- 'Post-treatment'
  postTrt_smmort$Category <- cat_nm
  
  out <- rbind(out, smallmort, study_smmort, postTrt_smmort)
  }else if(site == 'Teakettle'){
    postTrt_smmort <- get_cumul_mort(smallmort[smallmort$start_yr>trt_dates[[site]][['Thin']][[1]],],
                       group_cols = c('Treatment'))
    postTrt_smmort$Timeframe <- 'Post-treatment'
    ...
    out <- bind_rows(out, smallmort, study_smmort, postTrt_smmort)
    }else{
      out <- rbind(out, smallmort, study_smmort)
      }
...
for(y in 2:length(invntry_dates[[site]])){
    ...
    if(site=='Teakettle'){
      ...
      m <- make_mrtlty_tbl(raw[raw$DBH_BA>=size_thresh,], status_col=status_col, 
                       dead_ind = dead_ind, 
                       rndm_fx = rndm_fx)
    ...
    lgmort <- rbind(lgmort, m)
  }

lgmort <- lgmort[,!(colnames(lgmort) %in% c('df', 'asymp.LCL', 'asymp.UCL'))]
lgmort$Timeframe <- 'Period mortality'

### Get cumulative mortality for entire period
lgmort$survival = 1 - lgmort$prob

study_lgmort <- get_cumul_mort(lgmort, 
                             group_cols = c('Treatment'))
study_lgmort$Timeframe <- 'Study mortality'
study_lgmort$Category <- cat_nm

if(site=='Blodgett'){
  postTrt_lgmort <- get_cumul_mort(lgmort[lgmort$start_yr>trt_dates[[site]][['Thin']][[1]] & 
                       lgmort$end_yr < trt_dates[[site]][['Thin']][[2]] & 
                         lgmort$Treatment %in% c('Thin', 'None'),],
                       group_cols = c('Treatment'))
  postTrt_lgmort$Timeframe <- 'Post-treatment'
  postTrt_lgmort$Category <- cat_nm
  
  out <- rbind(out, lgmort, study_lgmort, postTrt_lgmort)
}else if(site == 'Teakettle'){
  postTrt_lgmort <- get_cumul_mort(lgmort[lgmort$start_yr>trt_dates[[site]][['Thin']][[1]],],
                       group_cols = c('Treatment'))
  postTrt_lgmort$Timeframe <- 'Post-treatment'
  ...
  out <- bind_rows(out, lgmort, study_lgmort, postTrt_lgmort)
}else{
  out <- rbind(out, lgmort, study_lgmort)
}
...
out$IncludesPreTreat <- 'No'

if(site=='STEF'){
  out[out$Treatment == 'Burn' & out$start_yr==2012, 'IncludesPreTreat'] = 'Yes'
}else if(site =='Blodgett'){
  out[out$start_yr==2001, 'IncludesPreTreat'] = 'Yes'
}else if(site == 'Teakettle'){
  out[out$start_yr <= 2001, 'IncludesPreTreat'] = 'Yes'
}

write.csv(out, file.path(paste0('../data/Benchmarking_data/Benchmarks/mortality_', site, '.csv')), row.names=FALSE)
```
...
write.csv(out_pre_post, file.path(paste0('../data/Benchmarking_data/Benchmarks/prePost_fuels_', site, '.csv')), row.names=FALSE)
