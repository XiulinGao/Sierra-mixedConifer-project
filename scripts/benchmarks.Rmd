...
make_benchmark_summary <- function(means_all_years, 
                                   group_cols, 
                                   var_nm, 
                                   nYr_df,
                                   ref_year){
  ## Subset final year values
  if(site=='Teakettle'){
    ## Assumes the final measurement year we benchmark is the DBH_10Post - DBH15Post period
    fnl_yr <- means_all_years[means_all_years$Timeframe == 'DBH_10Post',]
  }else{
    ## For other sites, we can just find the maximum year
    fnl_yr <- means_all_years[means_all_years$End_yr==as.character(max(as.numeric(means_all_years$End_yr))),]
  }
  
  
  ### Compute average difference over post-treatment period
  ## Exclude Control plots and pre-treatment timeframes
  ## This is okay for Teakettle if ref_year is set to 2001 and we don't include DBH_Pre
  avg_post <- means_all_years[means_all_years$Start_yr!=ref_year & 
                                means_all_years$Treatment!='None',]
  
  ## Look up number of years included in each measurement interval 
  avg_post$nYr <- avg_post$End_yr - avg_post$Start_yr
  
  # for(y in unique(avg_post$End_yr)){
  #   nyr <- unique(nYr_df[nYr_df$End_yr==y, 'nYr'])
  #   avg_post[avg_post$End_yr==y, 'nYr'] = nyr
  # }
  # 
  ## Weight each measurement interval average by the number of years in interval
  wghtd_post <- avg_post %>%
    group_by(across({{group_cols}})) %>% 
    summarise(Time_wghtd_avg = weighted.mean(percent_diff, nYr),
            SE_time_wghtd_avg = sqrt(sum(nYr^2 * se_percent_diff^2) / (sum(nYr)^2)))
  
  wghtd_post$Year_range <- NA
  for(trt in unique(wghtd_post$Treatment)){
    wghtd_post[wghtd_post$Treatment == trt, 'Year_range'] = paste0(
      as.character(min(avg_post[avg_post['Treatment']==trt, 'Start_yr'])), '-', as.character(max(avg_post[avg_post['Treatment']==trt, 'End_yr'])))
  }
  
  ### Merge final year and average benchmarks
  out <- merge(fnl_yr[, c(group_cols, 'emmean', 'SE', 
                               'percent_diff', 'se_percent_diff', 'End_yr')], 
               wghtd_post, 
               on = group_cols)
  
  ## Clean column names
  colnames(out)[colnames(out)=='emmean'] = var_nm
  colnames(out)[colnames(out)=='SE'] = paste0('SE_', var_nm)
  colnames(out)[colnames(out)=='percent_diff'] = 'Final_yr_pct_diff'
  colnames(out)[colnames(out)=='se_percent_diff'] = 'SE_final_yr'
...
if(site=='Teakettle'){
  ## Filter the pre-treatment and pre/post measurements for the second entry burn, for which we don't have a control
  avg_ba_by_unit <- compute_avgs_over_intervals(ba_by_unit[ba_by_unit$DBH_timeframe %in% c('DBH_Post', "DBH_10Post", "DBH_15Post"),], var = 'BAperTree_m2ha')
}else{
...
if(site=='Blodgett'){
  ref_year <- as.character(min(ba_by_unit$Year))
}else if (site=='STEF'){
  ref_year <- 2009
...
if(site == 'Teakettle'){
  for(time in c('DBH_Post', 'DBH_10Post')){
    ...
    year_outs[[1]]$Timeframe <- time
    
    # Add Year columns
    year_outs[[1]]$Start_yr = NA
    year_outs[[2]]$Start_yr = NA
    year_outs[[1]]$End_yr <- NA
    year_outs[[2]]$End_yr <- NA
    
    for(trt in unique(year_outs[[1]]$Treatment)){
      year_outs[[1]][year_outs[[1]]$Treatment==trt, 'Start_yr'] = 
        year_outs[[2]][year_outs[[2]]$Treatment==trt, 'Start_yr'] = 
        min(time_df[time_df$Treatment==trt, 'Start_yr'])
      
      year_outs[[1]][year_outs[[1]]$Treatment==trt, 'End_yr'] = 
        year_outs[[2]][year_outs[[2]]$Treatment==trt, 'End_yr'] = 
        max(time_df[time_df$Treatment==trt, 'End_yr'])
    }
  ...
  for(year in unique(avg_ba_by_unit$End_yr)){
    # Get Treatment means and contrasts for year
     year_outs <- make_emm_df(avg_ba_by_unit[avg_ba_by_unit$End_yr==year,],
                             compare_column = 'Treatment',
                             var = 'var', 
                             ref = 'None')
    # Add Year columns
    year_outs[[1]]$Start_yr <- unique(avg_ba_by_unit[avg_ba_by_unit$End_yr==year, 'Start_yr'])
    year_outs[[2]]$Start_yr <- unique(avg_ba_by_unit[avg_ba_by_unit$End_yr==year, 'Start_yr'])
     
    year_outs[[1]]$End_yr <- year
    year_outs[[2]]$End_yr <- year
...
ba_summary <- make_benchmark_summary(ba_means_all_years, 
                                     group_cols = c('Treatment'),
                                     var_nm = 'Basal_area_m2ha',
                                     nYr_df = avg_ba_by_unit,
                                     ref_year = ref_year)
...
ba_trt_diffs <- ggplot(ba_means_all_years[ba_means_all_years$Treatment!='None' & 
  ...
  # black line is 0
  geom_hline(yintercept = 0, lty = 3) + 
  geom_hline(yintercept = ba_summary[ba_summary$Treatment=='Burn', 'Time_wghtd_avg'], 
             color = trt_cols[['Burn']], alpha=0.5) +
    geom_hline(yintercept = ba_summary[ba_summary$Treatment=='Thin', 'Time_wghtd_avg'], 
             color = trt_cols[['Thin']], alpha=0.5) +
    geom_hline(yintercept = ba_summary[ba_summary$Treatment=='Burn+Thin', 'Time_wghtd_avg'], 
...
if(site == 'Teakettle'){
  ...
  pft_fracs <- pft_fracs[pft_fracs$DBH_timeframe %in% c('DBH_Pre', 'DBH_Post', "DBH_10Post", "DBH_15Post"),]
...
pft_contrasts_all_years <- data.frame()
...
if(site == 'Teakettle'){
  for(p in unique(all_pft_avgs$PFT)){
    for(time in c('DBH_Post', 'DBH_10Post')){
      ...
      # Bind to previous years
      pft_means_all_years <- rbind(pft_means_all_years, year_outs[[1]])
      pft_contrasts_all_years <- rbind(pft_contrasts_all_years, year_outs[[2]])
  ...
  for(p in unique(all_pft_avgs$PFT)){
    ...
    # Get Treatment means and contrasts for year
      year_outs <- make_emm_df(all_pft_avgs[all_pft_avgs$End_yr==year & 
                                              all_pft_avgs$PFT==p,],
                               compare_column = 'Treatment',
                               var = 'var', 
                               ref = 'None')
      # Add Year columns
      year_outs[[1]]$Start_yr <- unique(all_pft_avgs[all_pft_avgs$End_yr==year, 'Start_yr'])
      year_outs[[2]]$Start_yr <- unique(all_pft_avgs[all_pft_avgs$End_yr==year, 'Start_yr'])
...
pft_summary <- make_benchmark_summary(pft_means_all_years, 
...
if(site == 'Teakettle'){
  avg_tph_by_unit <- compute_avgs_over_intervals(tph_by_unit[tph_by_unit$DBH_timeframe %in% c('DBH_Pre', 'DBH_Post', "DBH_10Post", "DBH_15Post"),], var = 'TPH')
}else{
...
tph_contrasts_all_years <- data.frame()
...
if(site == 'Teakettle'){
  for(time in c('DBH_Post', 'DBH_10Post')){
    ...
    # Bind to previous years
    tph_means_all_years <- rbind(tph_means_all_years, year_outs[[1]])
    tph_contrasts_all_years <- rbind(tph_contrasts_all_years, year_outs[[2]])
  ...
  for(year in unique(avg_tph_by_unit$End_yr)){
    # Get Treatment means and contrasts for year
     year_outs <- make_emm_df(avg_tph_by_unit[avg_tph_by_unit$End_yr==year,],
                             compare_column = 'Treatment',
                             var = 'var', 
                             ref = 'None')
    # Add Year columns
    year_outs[[1]]$Start_yr <- unique(avg_tph_by_unit[avg_tph_by_unit$End_yr==year, 'Start_yr'])
    year_outs[[2]]$Start_yr <- unique(avg_tph_by_unit[avg_tph_by_unit$End_yr==year, 'Start_yr'])
...
```{r tph_summary}
tph_summary <- make_benchmark_summary(tph_means_all_years, 
                                     group_cols = c('Treatment'),
                                     var_nm = 'Stems_per_ha',
                                     nYr_df = avg_tph_by_unit,
                                     ref_year = ref_year)
...
tph_trt_diffs <- ggplot(tph_means_all_years[tph_means_all_years$Treatment!='None' & 
  ...
  # black line is 0
  geom_hline(yintercept = 0, lty = 3) + 
  geom_hline(yintercept = tph_summary[tph_summary$Treatment=='Burn', 'Time_wghtd_avg'], 
             color = trt_cols[['Burn']], alpha=0.5) +
    geom_hline(yintercept = tph_summary[tph_summary$Treatment=='Thin', 'Time_wghtd_avg'], 
             color = trt_cols[['Thin']], alpha=0.5) +
    geom_hline(yintercept = tph_summary[tph_summary$Treatment=='Burn+Thin', 'Time_wghtd_avg'], 
             color = trt_cols[['Burn+Thin']], alpha=0.5) +
  geom_point(position = position_dodge(dodge)) + geom_errorbar(position = position_dodge(dodge)) +
  scale_color_manual(values=unlist(trt_cols), name='Treatment', breaks = c('None', 'Burn', 'Thin', 'Burn+Thin')) +
  # geom_rect(data = ba_summary, aes(ymin = Time_wghtd_avg - SE_time_wghtd_avg,
...
if(site == 'Teakettle'){
  sizeDist_by_unit <- sizeDist_by_unit[sizeDist_by_unit$DBH_timeframe %in% c('DBH_Pre', 'DBH_Post', "DBH_10Post", "DBH_15Post"),]
}
...
size_contrasts_all_years <- data.frame()
...
if(site == 'Teakettle'){
  for(s in unique(all_size_avgs$Size_class)){
    for(time in c('DBH_Post', 'DBH_10Post')){
      ...
      # Add Size column
      year_outs[[1]]$Size_class <- s
      year_outs[[2]]$Size_class <- s
      
      # Bind to previous years
      size_means_all_years <- rbind(size_means_all_years, year_outs[[1]])
      size_contrasts_all_years <- rbind(size_contrasts_all_years, year_outs[[2]])
  ...
  for(s in unique(all_size_avgs$Size_class)){
    ...
    # Get Treatment means and contrasts for year
      year_outs <- make_emm_df(all_size_avgs[all_size_avgs$End_yr==year & 
                                              all_size_avgs$Size_class==s,],
                               compare_column = 'Treatment',
                               var = 'var', 
                               ref = 'None')
      # Add Year columns
      year_outs[[1]]$Start_yr <- unique(all_size_avgs[all_size_avgs$End_yr==year, 'Start_yr'])
      year_outs[[2]]$Start_yr <- unique(all_size_avgs[all_size_avgs$End_yr==year, 'Start_yr'])
...
# # Contrasts dataframe is mostly informational
# print(size_contrasts_all_years)
...
size_summary <- make_benchmark_summary(size_means_all_years, 
                                      group_cols = c('Treatment', 'Size_class'),
                                      var_nm = 'Stems_per_hectare', 
                                      nYr_df = all_size_avgs,
                                      ref_year = ref_year)
...
make_mrtlty_tbl <- function(df, status_col, dead_ind, ivl, t, rndm_fx = NULL){
...
if(site!='Teakettle'){
  ...
  for(y in 2:length(invntry_dates[[site]])){
    ...
    if(site=='STEF' & y == 2){
      m <- make_mrtlty_tbl(raw[raw$DBH_BA >= 15 & raw$Treatment %in% c('None', 'Thin'),], 
                           status_col = 'Status', 
                         dead_ind = dead_ind, ivl = ivl, 
                         t=invntry_dates[[site]][y]-invntry_dates[[site]][y-1], 
                         rndm_fx = 'Unit')
    }else{
  ...
  allmort <- allmort[,c('Treatment', 'rate', 'SE', 'asymp.LCL', 'asymp.UCL', 'PFT', 'calculationInterval', 'percent_diff', 'se_percent_diff')]
