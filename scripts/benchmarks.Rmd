...
if(site=='Teakettle'){
  ...
  pre <- sizeDist_by_unit[sizeDist_by_unit$Treatment == 'Burn' & 
                            sizeDist_by_unit$Year==2012,]
}else if(site == 'Blodgett'){
...
if(site == 'STEF'){
  for(s in unique(prepost$Size_class)){
    prepost_out <- make_emm_df(prepost[prepost$Treatment=='Burn' & prepost$Size_class==s,],
                                      compare_column = 'DBH_timeframe',
                                      var = 'var',
                                      ref = 'Pre',
                                      rndm_fx = 'Unit')
    
    prepost_benchmarks <- calculate_simple_diff(prepost_out,
                                            compare_column = 'DBH_timeframe',
                                            ref = 'Pre',
                                            out_col = 'emmean')
    ...
    prepost_out$Size_class <- s
    
    prepost_by_size <- rbind(prepost_by_size, prepost_out)
  }
  }else{
    for(s in unique(prepost$Size_class)){
      for(trt in c('Burn', 'Thin', 'Burn+Thin')){
