...
main_path   = file.path("/Volumes/Ldian/FATES/sierra-refine/fireon")
#fireon_path = file.path(main_path,"fireon-site/2022-11-02")
util_path   = file.path(home_path,"Util/RUtils")

ens_cases  = c("cz2-p61-finetune.nc",
               "tharp-p61-finetune.nc"
               )
...
#---~---
  for (p in sequence(nbypft-n_varpft_last)){
      ...
      ncm_dat      = aperm(ncm_dat,c(2,3,1))
      dim(ncm_dat) = c(ninst*ntstamp_each, ndbhs*npfts)
      p1           = base::rowSums(ncm_dat[,2:13]) # for woody PFT we only sums size class > 2.5cm
      p2           = base::rowSums(ncm_dat[,15:26])
      p3           = base::rowSums(ncm_dat[,28:39])
      p4           = base::rowSums(ncm_dat[,41:52])
...
#bypft  =     bypft                                              %>% 
#             group_by(case,time)                                %>% 
#             mutate(tnplant = sum(fates_nplant,na.rm=TRUE))     %>% 
#             ungroup()
...
```{r,label="ensemble-filtering"}

ens_filter =   pft_strucmean                                %>% 
               group_by(case,ens_label,case_ens)            %>% 
               mutate(survi_lab = (fates_vegc_aboveground[pft==1] >1.5 &
                                   fates_vegc_aboveground[pft==2] >0.6 &
                                   fates_vegc_aboveground[pft==3] >1 &
                                   fates_vegc_aboveground[pft==4] >0.1)) %>% 
...
```{r,label="climate-fire-PFT-composition"}
comp_var = c("fates_crownarea","fates_basalarea",
             "fates_nplant","fates_vegc_aboveground")
...
mort_var = mortout_pft[c(1,4:7,11:12)]
  ...
  mort_labs   = c("BgMort","CstrMort","FireMort","FrezMort","HydroMort","TermMort","UstoryMort")
...
for(n in sequence(ncase)){
  ...
  plot1 = ggplot(szpf_survi,aes(dbh,nplant)) + geom_bar(stat = "identity") +
       facet_wrap(ens_label~pft,scales="free",labeller=labeller(pft=s_pftlabs)) + 
...
for(n in sequence(ncase)){
  ...
  for (m in mort_which){
    ...
    mort_pfsz = mort_df                             %>% 
                ...
                group_by(ens_label,pft,dbh)         %>% 
                select(-c(time,month,year,nplant_ustory,nplant)) %>% 
                summarize_all(mean)                 %>% 
                ungroup()    
  ...
  plot = ggplot(mort_pfsz,aes_string("dbh",m_vnam)) + geom_bar(stat = "identity") +
       facet_wrap(ens_label~pft, labeller=labeller(pft=pft_labs)) + 
       scale_x_discrete(labels = dbh_labs ) +
       labs(title=desc_now,
  ...
  for (d in sequence(ndevice)){
    f.output = paste0(case_now,m_vnam,"-pfsz.",gg_device[d])
    dummy = ggsave( filename = f.output
                  , plot     = plot
                  , device   = gg_device[d]
                  , path     = tsmort_path
...
for(a in sequence(length(plot_age))){
   ...
   age_df   = age_df                       %>% 
              mutate(year = year(time))    %>% 
              filter(ens_label %in% survi_lab,
                     year %in% c(lst_s:lst_e)) %>% 
              group_by(case,ens_label,pft,
                       age)                %>% 
              dplyr::select(-c(time,year)) %>% 
              summarize_all(mean,
                            na.rm=TRUE)    %>% 
   ...
   gg_now = gg_now + geom_bar(stat = "identity")
   gg_now = gg_now + facet_wrap(case~ens_label,nrow=ncase,scales="free",labeller=labeller(case=case_desc))
