...
ddbh_vars = names(bypft)[grepl("ddbh",names(bypft))]
n_ddbh = length(ddbh_vars)
for(v in sequence(n_ddbh)){
  var_now = ddbh_vars[v]
  bypft[[var_now]] = ddbh_scale_fun(bypft,var_now)
}
...
ddbh_vars = names(overall)[grepl("ddbh",names(overall))]
n_ddbh = length(ddbh_vars)
for(v in sequence(n_ddbh)){
  var_now = ddbh_vars[v]
  overall[[var_now]] = ddbh_scale_fun(overall,var_now)
}
...
for(d in sequence(n_delta)){
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"_byTreatment-smoothed.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = plot
                    , device   = gg_device[d]
                    , path     = mean_path
...
ddbh_vars = names(bydbh)[grepl("ddbh",names(bydbh))]
n_ddbh = length(ddbh_vars)
for(v in sequence(n_ddbh)){
  var_now = ddbh_vars[v]
 bydbh[[var_now]] = ddbh_scale_fun(bydbh,var_now)
}
