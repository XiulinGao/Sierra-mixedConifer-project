...
bypft_zero = bypft                                                                     %>% 
             mutate(fates_ddbh_canopy = fates_ddbh_canopy*ha_2_m2*fates_nplant_canopy,
             fates_ddbh_ustory = fates_ddbh_ustory*ha_2_m2*fates_nplant_ustory,
             fates_ddbh        = fates_ddbh*ha_2_m2*fates_nplant)
...
for(d in sequence(n_delta)){
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"_PFTSplit-smoothed-15.",gg_device[d])
...
for(d in sequence(n_delta)){
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"_byTreatment-smoothed-15-dbh.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = plot
                    , device   = gg_device[d]
                    , path     = mean_path
