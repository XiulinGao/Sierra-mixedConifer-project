...
for(d in sequence(n_delta)){
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"_PFTSplit-outlierRemoved.",gg_device[d])
...
for(v in sequence(n_delta)){
  ...
  delta_growth[[var_now]] = delta_pct_fun(delta_growth,var_now,0.9,0.1)
}
...
for(d in sequence(n_delta)){
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"_deltabySZ-outlierRemoved.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = plot
                    , device   = gg_device[d]
                    , path     = size_path
