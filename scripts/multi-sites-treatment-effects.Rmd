...
obs_main = file.path("Git-repos/Sierra-mixedConifer-project/data/Benchmarking_data/Benchmarks")
...
fuel_obs = c("fuels_Blodgett.csv",
             #"fuels_STEF.csv",
             "fuels_Teakettle.csv")
comp_obs = c("pft_composition_Blodgett.csv",
             "pft_composition_STEF.csv",
             "pft_composition_Teakettle.csv")
...
prepost_comp_obs = c("prepost_pft_composition_Blodgett.csv",
                     "prepost_pft_composition_STEF.csv",
                     "prepost_pft_composition_Teakettle.csv")
...
comp_path = file.path(obs_path,comp_obs)
...
diff_comp_path = file.path(obs_path,prepost_comp_obs)
...
mort_fun2 = function(x,n){
  mort_pcnt = ifelse(test = n ==0, yes=NA, no=(x/n)*100)
  return(mort_pcnt)
}
...
mutate(fates_ddbh_canopy = ifelse(is.na(fates_nplant_canopy) | fates_nplant_canopy==0,
       ...
       fates_mortality = ifelse(is.na(fates_nplant) | fates_nplant==0,
                                NA, fates_mortality/fates_nplant*100),
...
bydbh$fates_mortality = bydbh %>% dplyr::select(starts_with("fates_mortality")) %>% rowSums()
...
all_obs_paths = c("ba_path","nplant_path","ctrl_growth_path","delta_growth_path","mort_path","sz_path","fuel_path",
                  "comp_path","diff_ba_path","diff_sd_path","diff_szdist_path","diff_fuel_path","diff_comp_path")
obs_file_name = c("ba-obs.csv","nplant-obs.csv","ctrl-ddbh-obs.csv","ddbh-obs.csv","mort-obs.csv","szdist-obs.csv",
                  "fuel-obs.csv","comp-obs.csv","prepost-ba-obs.csv","prepost-sd-obs.csv","prepost-szdist-obs.csv",
                  "prepost-fuel-obs.csv","prepost-comp-obs.csv"
                  )
...
model_ctrl_struc_bldgt = overall_na                            %>% 
                         select(case,desc,year,
                                fates_basalarea,
                                fates_nplant)                  %>% 
                         filter(case ==1 &
                                year %in% c(2003:2020)) 
model_ctrl_struc_stef = overall_na                            %>% 
                         select(case,desc,year,
                                fates_basalarea,
                                fates_nplant)                  %>% 
                         filter(case ==5 &
                                year %in% c(2012:2018)) 
model_ctrl_struc_tea = overall_na                            %>% 
                         select(case,desc,year,
                                fates_basalarea,
                                fates_nplant)                  %>% 
                         filter(case == 9 &
                                year %in% c(2002:2019))
model_ctrl_struc = rbind(model_ctrl_struc_bldgt, model_ctrl_struc_stef, model_ctrl_struc_tea)
...
for(n in sequence(n_plot)){
  ...
  gg_now <- gg_now + geom_errorbar(aes(ymin=fates_nplant - fates_nplant_se,
                     ...
                     width=0.2,
                     position=position_dodge(0.6),stat="identity")}
  gg_now = gg_now + scale_fill_manual(values=c("black","grey50"))
...
gg_patch = wrap_plots(delta_ba_plot, delta_stem_plot) +
  plot_layout(guides="collect",ncol=2) +
...
for (d in sequence(ndevice)){
    f_output = paste0("delta-struc_benchmark.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = gg_patch
                    , device   = gg_device[d]
                    , path     = bncmark_path
                    , width    = gg_widthn*0.65
                    , height   = gg_height*0.75
                    , units    = gg_units
                    , dpi      = gg_depth
                    )
...
ctrl_sz_mod_bldgt = sz_model                         %>% 
              filter(case %in% c(1) &
                      year %in% c(2003:2020))        %>% 
              select(case,desc,dbh,fates_nplant)     
ctrl_sz_mod_stef = sz_model                         %>% 
              filter(case %in% c(5) &
                      year %in% c(2012:2018))        %>% 
              select(case,desc,dbh,fates_nplant)     
ctrl_sz_mod_tea = sz_model                         %>% 
              filter(case %in% c(9) &
                      year %in% c(2002:2019))        %>% 
              select(case,desc,dbh,fates_nplant)  
ctrl_sz_mod_all = rbind(ctrl_sz_mod_bldgt, ctrl_sz_mod_stef,ctrl_sz_mod_tea)
ctrl_sz_mod = ctrl_sz_mod_all                        %>% 
              group_by(case,desc,dbh)                %>% 
              summarize_all(mean,na.rm=TRUE)         %>% 
              ungroup()
ctrl_sz_mod_se = ctrl_sz_mod_all                     %>% 
              group_by(case,desc,dbh)                %>% 
...
gg_now = gg_now + ylab(expression("Stem density by size class"~"( Plants"~ha^-1~")"))
...
gg_now = gg_now + facet_wrap(site~pft, labeller=labeller(pft=pft_keys), ncol=3,scales="free_y")
gg_now = gg_now + geom_bar(position=position_dodge(),stat="identity",width=0.6) 
gg_now = gg_now + geom_errorbar(aes(ymin=fates_ddbh-lwr_ddbh,
                         ymax=fates_ddbh+upr_ddbh),
                     width=0.2,
                     position=position_dodge(0.6))
#gg_now = gg_now + geom_line(aes(x = dbh, y = fates_ddbh, group=type, colour=type))
gg_now = gg_now + geom_smooth(aes(group=type, color=type), method="loess",se=FALSE, na.rm=TRUE,
                              linetype=1, linewidth=0.8)
gg_now = gg_now + scale_fill_manual(values=c("black","grey50"))
gg_now = gg_now + scale_color_manual(values=c("black","grey50"))
gg_now = gg_now + scale_x_discrete(labels=dbh_labs[4:13])
...
## benchmark relative change in growth rate only for plant within 30-40cm group
## for treatments

## this is for calculating relative growth rate using averaged PFT growth
...
## decide which data to use: PFT mean growth or growth for size group 30-40cm
# ddbh_df = ddbh_df
ddbh_df = ddbh_szpf_mod %>% filter(dbh==6)
...
ddbh_pft_plot  = ggplot(plot_df,aes(x=pft,y=fates_ddbh,
                   fill=type)) +
       facet_wrap(treatment~site, scales="free_y") +              
       geom_bar(position=position_dodge(),stat="identity",width=0.6) +
       geom_errorbar(aes(ymin=lwr_ddbh,
                         ymax=upr_ddbh),
                     width=0.2,
                     position=position_dodge(0.6))+
       geom_hline( yintercept = 1, color = "darkred", linetype = 2, linewidth=0.8)+
       scale_x_discrete(labels=pft_labs)+
       scale_fill_manual(values=c("Model-BurnCool" = "#836B43",
                                  "Model-BurnHot" = "#D68D18",
                                  "Model-Thin" = "#A0AE6A",
                                 "Observation" = "#437683"))+
       ylab("Treated to control growth rate multiplier") +
       xlab("") +
       theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size = 0.5,base_rect_size =0.5) +
       theme( legend.title =element_blank()
            , legend.position="bottom"
...
mod_mortpft = mod_mortpft                    %>% 
              #mutate_at(vars(fates_mortality), 
              #~ mort_fun2(.x,fates_nplant))  %>% 
              group_by(case,desc,year,pft)   %>% 
              summarize_all(mean,na.rm=TRUE) %>% 
              ungroup()                      %>% 
              mutate_at(vars(fates_mortality),
...
mod_mortpft_yr = mod_mortpft_yr %>% select(-fates_nplant)   
mod_mortpft_sum = mod_mortpft_sum %>% select(-fates_nplant)
...
mod_mort_dbh  = bydbh_na                                  %>% 
                select(all_of(c("case","desc",
                                "year","dbh","fates_nplant",
                              mortout_dbh[c(1:6,9:10)]))) %>% 
...
## then calculate mortality rate for each size group (DBH <=60 & DBH >60)
mod_mort_dbh  = mod_mort_dbh                                   %>% 
                group_by(case,desc,site,year,dbh_grp)          %>% 
                mutate(fates_nplant = sum(fates_nplant,na.rm=TRUE),
                       fates_mortality = sum(fates_mortality, na.rm=TRUE)) %>% 
                ungroup()                                      %>% 
                mutate_at(vars(fates_mortality), 
                          ~ mort_fun2(.x, fates_nplant))       

## then sum across each time window to get cumulative mortality rate
## for each of the two size groups at each site
bldgt = mod_mort_dbh                       %>% 
        filter(site=="Blodgett")           %>% 
        mutate(yr_group = cut(year,breaks=c(-Inf,2003,2009,2016,Inf),
                              labels=c("2001-2003","2004-2009",
...
stef = mod_mort_dbh                         %>% 
       filter(site=="STEF" &
              year %in% c(2011:2018))       %>%   ## change this to 2011:2018, was 2012:2018
       select(case,desc,site,dbh_grp,
              fates_mortality)              %>% 
       distinct()                           %>% 
       group_by(case,desc,site,dbh_grp)     %>% 
...
mortsum_plot  = ggplot(plot_df,aes(x=dbh_grp,y=fates_mortality,
                   fill=type)) +
       facet_wrap(site~treatment,ncol=3, scales="free_y") +
       geom_bar(position=position_dodge(),stat="identity",width=0.6) +
       geom_errorbar(aes(ymin=fates_mortality-se_mortality,
                         ymax=fates_mortality+se_mortality),
                    width=0.2,
...
bldgt_fuel_ctrl = bldgt_fuel_ctrl %>% left_join(bldgt_fuel_se,
                                                by=c("case","desc","fuel"))
bldgt_fuel_ctrl = bldgt_fuel_ctrl %>% filter(fuel %in% c(1:3,5))
...
obs_fuel_ctrl = obs_fuel                           %>% 
 ...
 obs_fuel_ctrl = obs_fuel_ctrl                             %>% 
                 ...
                 rename(treatment = Treatment,
                        ...
                        se = SE)    %>% 
                 mutate(fuel = case_when(fuel %in% c("ONE_HOUR_FUELS","1-hr","1h_Mg_ha") ~ "1",
                                         fuel %in% c("TEN_HOUR_FUELS","10-hr","10h_Mg_ha") ~ "2",
                                         fuel %in% c("HUNDRED_HOUR_FUELS","100-hr","100h_Mg_ha") ~ "3",
                                         fuel %in% c("Litter","litter_Mg_ha") ~ "5")) %>% 
  ...
  if(n %in% c(2:4)){
    ...
    if(n==2){
     ...
     if(n==6){
       ...
       fst_yr = 2012      ## use 2012 data for burning and 2014 data for thinning
       lst_yr = 2012
     }
...
obs_deltafuel = obs_fuel                               %>% 
                filter(Year %in% c(2003,2014),
                      Fuel_class %in% c("litter_Mg_ha",
                                        "Total_1_10_100_hr_fuels",
                                        "Litter") &
                        Treatment %in% c("Burn","Thin"))  %>% 
                select(site,Treatment,Fuel_class,
                       percent_diff,se_percent_diff)   %>% 
                mutate(Fuel_class = ifelse(Fuel_class == "litter_Mg_ha", "Litter", Fuel_class))%>% 
...
#wrap all together
gg_patch = wrap_plots(delta_plot[[3]], delta_plot[[2]]) +
  plot_layout(guides="collect",ncol=2) +
   plot_annotation(tag_levels="a") & 
  theme(plot.tag.position = c(-0.02, 1.02),
        legend.position="bottom")

for (d in sequence(ndevice)){
    f_output = paste0("delta-fuel_benchmark.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = gg_patch
                    , device   = gg_device[d]
                    , path     = bncmark_path
                    , width    = gg_widthn*0.7
                    , height   = gg_height*0.72
                    , units    = gg_units
                    , dpi      = gg_depth
                    )
 }
...
### baseline PFT composition benchmark
comp_mod = bypft                                         %>% 
           select(case,desc,time,pft,fates_basalarea)    %>% 
           group_by(case,desc,time)                      %>% 
           mutate(total_ba = sum(fates_basalarea))       %>% 
           ungroup()                                     %>% 
           mutate(fates_basalarea = 
                    (fates_basalarea/total_ba)*100)      %>% 
           select(-total_ba)
comp_ctrl_bldgt = comp_mod                               %>% 
                  filter(case == 1)                      %>% 
                  mutate(year = year(time))              %>% 
                  filter(year %in% c(2003:2020))         %>% 
                  select(-c(time,year))                  %>% 
                  group_by(case,desc,pft)                %>% 
                  summarize_all(mean, na.rm=TRUE)       %>% 
                  ungroup()

comp_ctrl_se_bldgt = comp_mod                            %>% 
                  filter(case == 1)                      %>% 
                  mutate(year = year(time))              %>% 
                  filter(year %in% c(2003:2020))         %>% 
                  select(-c(time,year))                  %>% 
...
comp_ctrl_se_tea = comp_mod                              %>% 
                  ...
                  summarize_each(funs(se=sd(.)/sqrt(n()))) %>%         
                  ungroup()
comp_ctrl_bldgt = comp_ctrl_bldgt %>% left_join(comp_ctrl_se_bldgt,by=c("case","desc","pft"))
comp_ctrl_stef = comp_ctrl_stef %>% left_join(comp_ctrl_se_stef,by=c("case","desc","pft"))
comp_ctrl_tea = comp_ctrl_tea %>% left_join(comp_ctrl_se_tea,by=c("case","desc","pft"))
comp_ctrl_mod = rbind(comp_ctrl_bldgt,comp_ctrl_stef,comp_ctrl_tea)

comp_ctrl_mod = comp_ctrl_mod                           %>% 
                mutate(site=site_grab(desc),
                       type = "Model")                  %>% 
                select(site,pft,fates_basalarea,se,type)      
            

pft_comp_obs = fread(file.path(obs_path,"comp-obs.csv"))

comp_ctrl_obs = pft_comp_obs                      %>% 
                filter(Treatment=="None" &
                       PFT %in% c("cedar",
                                  "fir",
                                  "pine",
                ...
                mutate(PFT = case_when(PFT=="pine" ~ 1,
                                       ...
                                       PFT=="fir"~ 3,
                                       PFT=="oak"~ 4))        %>% 
                select(site,PFT,Avg,SE_Avg)       %>% 
                rename(pft = PFT,
                       fates_basalarea = Avg,
                       se = SE_Avg)               %>% 
                mutate(type = "Observation",
                       fates_basalarea = fates_basalarea*100,
                       se = se*100) 
 
comp_ctrl_all = rbind(comp_ctrl_mod, comp_ctrl_obs)   

plot_df = comp_ctrl_all
plot_df$pft = factor(plot_df$pft,levels=unique(plot_df$pft))
pft_keys = pftinfo$short[1:4]
names(pft_keys) = c(1:4)
plot_df = plot_df %>% filter(pft!=4)

gg_now = ggplot(plot_df,aes(x=pft,y=fates_basalarea,fill=type)) 
gg_now = gg_now + facet_wrap(.~site, ncol=3)
gg_now = gg_now + geom_bar(position=position_dodge(),stat="identity",width=0.6) 
gg_now = gg_now + geom_errorbar(aes(ymin=fates_basalarea-se,
                         ymax=fates_basalarea+se),
                     width=0.2,
                     position=position_dodge(0.6))
gg_now = gg_now + scale_fill_manual(values=c("black","grey50"))
gg_now = gg_now + scale_x_discrete(labels=pft_keys[1:3])
gg_now = gg_now + ylab("Percent basal area by PFT (%)")
gg_now = gg_now + xlab("") 
gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",
                                base_line_size = 0.5,base_rect_size =0.5) 
gg_now = gg_now + theme( legend.title =element_blank()
...
for (d in sequence(ndevice)){
    f_output = paste0("comp_ctrl_benchmark.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = gg_now
                    , device   = gg_device[d]
                    , path     = bncmark_path
...
### relative change in PFT composition 

ctrl_comp = comp_mod %>% filter(case %in% c(1,5,9))
ctrl_comp = ctrl_comp                      %>% 
            group_by(case,desc)            %>% 
            slice(rep(seq_len(n()),times = 3)) %>% ungroup()


comp_vars = c("fates_basalarea")

n_comp = length(comp_vars)


ctrl_comp =  ctrl_comp                    %>%
             select(all_of(comp_vars))    %>% 
             rename_at(vars(comp_vars),rename_fun)

treat_comp = comp_mod                                %>% 
             filter(!case%in% c(1,5,9))              %>% 
             select(case,desc,time,pft,
                    fates_basalarea)
  
delta_comp = cbind(treat_comp,ctrl_comp)

delta_comp = delta_comp                    %>% 
             mutate(site=site_grab(desc))

delta_comp$treatment = unlist(lapply(delta_comp$desc,treatment_grab))


for(v in sequence(n_comp)){
  var_now = comp_vars[v]
  delta_comp[[var_now]] = delta_pct_fun(delta_comp,var_now,1,0)
}
...
for(n in treated_case){
  delta_now = delta_comp %>% filter(case==n)
  if(n %in% c(2:4)){
    fst_yr = 2003
    lst_yr = 2020  ## 2003-2020 at Bldgt
  }else if(n %in% c(6:8)){
     if(n==6){
       fst_yr = 2012  ##2012-2018 for thinning at STEF
       lst_yr = 2018
     }else{
       fst_yr = 2014      ## 2014-2018 for burning at STEF
       lst_yr = 2018
     }

  }else if(n %in% c(10:12)){
    fst_yr = 2003
    lst_yr = 2019     ## 2003-2019 at Teakettle
  }
  mean_comp  = delta_now                                            %>% 
              mutate(year=year(time))                               %>% 
              filter(year%in%c(fst_yr,lst_yr))                      %>% 
  ...
  deltacomp_mean = rbind(deltacomp_mean,mean_comp)
  ...
  se_comp  = delta_now                                             %>%
             ...
             group_by(site,treatment,pft)                          %>% 
             summarize_all(funs(se=sd(.)/sqrt(n())))
  deltacomp_se = rbind(deltacomp_se,se_comp)
    
}

mod_deltacomp = deltacomp_mean                         %>% 
                select(site,treatment,pft,
                       fates_basalarea)                 
               
mod_deltacomp_se = deltacomp_se                        %>% 
                   select(site,treatment,pft,
                          fates_basalarea_se)            
                  
mod_deltacomp = mod_deltacomp %>% left_join(mod_deltacomp_se, by=c("site","treatment","pft"))
mod_deltacomp = mod_deltacomp                           %>% 
                mutate(type = case_when(treatment=="Burn-Cool" ~ "Model-BurnCool",
                                      treatment=="Burn-Hot" ~ "Model-BurnHot",
                                      treatment=="Thin" ~ "Model-Thin"),
                      treatment = ifelse(treatment %in% c("Thin","None"), treatment, "Burn"))

obs_deltacomp =  pft_comp_obs                 %>% 
                 select(site, Treatment, PFT,
                        Avg_pct_diff,
                        SE_Avg_pct_diff)      %>% 
                 filter(PFT %in% c("pine",
                                   "cedar",
                                   "fir",
                                   "oak") &
                        !Treatment %in% c("None","Burn+Thin")) %>% 
                 rename(pft = PFT,
                        treatment = Treatment,
                        fates_basalarea = Avg_pct_diff,
                        fates_basalarea_se = SE_Avg_pct_diff) %>% 
                 mutate(pft = case_when(pft=="pine"~ 1,
                                        ...
                                        pft=="oak"~4),
                        type = "Observation")
deltacomp_all = rbind(mod_deltacomp,obs_deltacomp)
plot_df = deltacomp_all
plot_df = plot_df %>% filter(pft!=4)
plot_df$pft = factor(plot_df$pft,levels=unique(plot_df$pft))
gg_now = ggplot(plot_df,aes(x=pft,y=fates_basalarea,fill=type)) 
gg_now = gg_now + geom_bar(position=position_dodge(),stat="identity",width=0.6) 
gg_now = gg_now + geom_errorbar(aes(ymin=fates_basalarea - fates_basalarea_se,
                     ...
                     position=position_dodge(0.6))
gg_now = gg_now + facet_wrap(treatment~site,ncol=3, scales="free_y")
gg_now = gg_now + scale_x_discrete(labels=pft_keys)
gg_now = gg_now + scale_fill_manual(values=c("Model-BurnCool" = "#836B43",
                                              "Model-BurnHot" = "#D68D18",
                                              "Model-Thin" = "#A0AE6A",
                                              "Observation" = "#437683"))
gg_now = gg_now + ylab("Relative change in basal area by PFT (%)")
gg_now = gg_now + xlab("")
gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",
                                base_line_size = 0.5,base_rect_size =0.5) 
gg_now = gg_now + theme( legend.title =element_blank()
 ...
 for (d in sequence(ndevice)){
    f_output = paste0("comp_delta_benchmark.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = gg_now
                    , device   = gg_device[d]
                    , path     = bncmark_path
...
## Blodgett
mod_prethin_comp_bldgt = comp_mod                     %>%
                         mutate(year = year(time))    %>% 
                         filter(year == 2001 &
                                case == 2)            %>% 
                         select(case,desc,
...
mod_postthin_comp_bldgt = comp_mod                    %>% 
                      mutate(year = year(time))       %>% 
                      filter(year != 2001 &
                             case == 2 )              %>% 
                      select(case,desc,pft,
                      ...
                      summarize_all(mean,na.rm=TRUE)  %>% 
                      ungroup()                       %>%  
                      rename(post_basalarea = fates_basalarea)

mod_postrx_comp_bldgt =  comp_mod                     %>%
                      mutate(year = year(time))       %>% 
                      filter(year != 2001 &
                             case %in% c(3:4))        %>% 
                      select(case,desc,pft,
...
mod_prethin_comp_stef = comp_mod                      %>% 
                      mutate(year = year(time),
                             month = month(time))     %>% 
                      filter(year == 2011 &
                             month == 1 &
...
mod_postthin_comp_stef = comp_mod                     %>% 
                      mutate(year = year(time))       %>% 
                      filter(year != 2011 &
                             case == 6 )              %>% 
                      select(case,desc,pft,
...
mod_postthin_comp_tea = comp_mod                     %>%  
                      mutate(year = year(time))      %>% 
                      filter(year != 2001 &
                             case == 10 )            %>% 
                      select(case,desc,pft,
...
mod_pre_comp = rbind(mod_prethin_comp_bldgt,mod_prerx_comp_bldgt,
                      mod_prethin_comp_stef,mod_prerx_comp_stef,
                      mod_prethin_comp_tea,mod_prerx_comp_tea)
mod_post_comp = rbind(mod_postthin_comp_bldgt,mod_postrx_comp_bldgt,
                       mod_postthin_comp_stef, mod_postrx_comp_stef,
                       mod_postthin_comp_tea, mod_postrx_comp_tea)

mod_prpo_comp = mod_pre_comp %>% left_join(mod_post_comp,by=c("case","desc","pft"))

prpo_vars = names(mod_prpo_comp)[grepl("fates_",names(mod_prpo_comp))]
n_var = length(prpo_vars)
for(n in sequence(n_var)){
  var_now = prpo_vars[n]
  mod_prpo_comp[[var_now]] = prepost_pct_diff(mod_prpo_comp,var_now)
  
}
mod_prpo_comp = mod_prpo_comp %>% mutate(site=site_grab(desc))
mod_prpo_comp$treatment = unlist(lapply(mod_prpo_comp$desc,treatment_grab))
mod_prpo_comp$fates_basalarea_se = NA


mod_prpo_comp = mod_prpo_comp                              %>% 
                mutate(type = case_when(treatment=="Burn-Cool" ~ "Model-BurnCool",
                                      treatment=="Burn-Hot" ~ "Model-BurnHot",
                                      treatment=="Thin" ~ "Model-Thin"),
                      treatment = ifelse(treatment=="Thin", treatment, "Burn"))
mod_prpo_comp = mod_prpo_comp %>% select(-c(case,desc,post_basalarea))
 
obs_prpo_comp = fread(file.path(obs_path,"prepost-comp-obs.csv"))


obs_prpo_comp = obs_prpo_comp               %>% 
                filter(DBH_timeframe == "Post" &
                     !Treatment %in% c("Burn+Thin","None"))  %>% 
                select(site,Treatment,
                       PFT,
...
mod_prpo_comp = rbind(mod_prpo_comp,obs_prpo_comp)

plot_df =mod_prpo_comp
plot_df$pft = factor(plot_df$pft,levels=unique(plot_df$pft))
plot_df = plot_df %>% filter(pft!=4)

max_y = max(mod_prpo_comp$fates_basalarea,na.rm=TRUE) + max(mod_prpo_comp$fates_basalarea_se,na.rm=TRUE) 
min_y = min(mod_prpo_comp$fates_basalarea,na.rm=TRUE) + min(mod_prpo_comp$fates_basalarea_se,na.rm=TRUE) 
gg_now = ggplot(plot_df,aes(x=pft,y=fates_basalarea,fill=type)) 
gg_now = gg_now + geom_bar(position=position_dodge2(),stat="identity",width=0.6)
gg_now = gg_now + geom_errorbar(aes(ymin=fates_basalarea - fates_basalarea_se,
                                    ymax=fates_basalarea + fates_basalarea_se),
...
gg_now = gg_now + scale_fill_manual(values=c("Model-BurnCool" = "#836B43",
                                              ...
                                              "Observation" = "#437683"))
gg_now = gg_now + scale_x_discrete(labels=pft_keys[1:3])
gg_now = gg_now + ylab("Pre-post change in percent basal area by PFT ( % )")
gg_now = gg_now + xlab("") 
gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",
                              base_line_size = 0.5,base_rect_size =0.5) 
gg_now = gg_now + theme( legend.title =element_blank()
...
for (d in sequence(ndevice)){
    f_output = paste0("comp_prepost_benchmark.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = gg_now
                    , device   = gg_device[d]
                    , path     = bncmark_path
...
#### burn window ####
hlm1d = fread(file.path(main_path,"hlm1d-3sites.csv"))

hlm1d$treatment = unlist(lapply(hlm1d$desc,treatment_grab))

hlm1d = hlm1d %>% mutate(site=site_grab(desc),
                         precip = rain + snow)

plot_df = hlm1d                                  %>% 
          filter(!case %in% c(1:2,5:6,9:10))     %>% 
          mutate(month = month(time))            %>% 
          select(site,desc,treatment,month,
                 fates_rx_burn_window)           %>% 
          group_by(site,desc,treatment,month)    %>% 
          summarize_all(mean,na.rm=TRUE)         %>% 
          ungroup()

plot = ggplot(plot_df)  +
       geom_line(aes(x=month,y=fates_rx_burn_window,
       ...
       scale_colour_manual(values=c("Burn-Cool" = "#836B43",
                                    "Burn-Hot" = "#D68D18")) +
       ylab(paste0("Prescribed fire burn window (%)")) +
       xlab("Month") +
       theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size = 0.5,base_rect_size =0.5) +
       theme( legend.title =element_blank()
            , legend.position="bottom"
  ...
  for (d in sequence(ndevice)){
    f_output = paste0("burn-window-by-site.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = plot
                    , device   = gg_device[d]
                    , path     = plot_main
...
plot_df = hlm1d                             %>% 
          filter(case %in% c(1,5,9))        %>% 
          mutate(month = month(time))       %>% 
          select(site, month,precip, tsa)   %>% 
          group_by(site,month)              %>% 
          summarize_all(mean,na.rm=TRUE)    %>% 
          ungroup()
plot_vars = c("precip", "tsa")
n_plot = length(plot_vars)

for (p in seq(n_plot)){
  var_now = plot_vars[p]
  plot = ggplot(plot_df)  +
         geom_line(aes_string(x="month",y=var_now,
                 colour="site",group="site"),
                 linewidth=1.5,alpha=0.5,show.legend=TRUE) +
       scale_x_continuous( breaks = sequence(12)) +
       scale_colour_manual(values=top12) 
  if(p==1){
    plot = plot + ylab("Monthly mean precipitation (mm)")
    f_output = paste0("precip-by_site.",gg_device[d])
  }else{
    plot = plot + ylab(expression("Monthly mean temperature ("~degree*"C"~")"))
    f_output = paste0("temp-by_site.",gg_device[d])
  }
  plot = plot + 
         xlab("Month") +
         theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size = 0.5,base_rect_size =0.5) +
...
### PFT fire mortality for prescribed fire simulations across sites ####
plot_df = bypft                               %>% 
          filter(!case %in% c(1:2,5:6,9:10))  %>% 
          mutate(site = site_grab(desc))
plot_df$treatment = unlist(lapply(plot_df$desc,treatment_grab))
pft_keys = pftinfo$short[1:4]
names(pft_keys) = c(1:4)

plot_df = plot_df                             %>% 
          select(site,treatment,year,pft,
                 fates_mortality_rxfire,
                 fates_nplant)                %>% 
          group_by(site,treatment,year,pft)   %>% 
          summarize_all(mean,na.rm=TRUE)      %>% 
          ungroup()                           %>% 
          mutate(fates_mortality_rxfire =
          fates_mortality_rxfire/fates_nplant*100) %>% 
          group_by(site,treatment,pft)        %>% 
          select(-c(year,fates_nplant))       %>% 
          summarize_all(sum,na.rm=TRUE)      %>% 
          ungroup()

plot_df$pft = factor(plot_df$pft,levels=unique(plot_df$pft))
 
gg_now = ggplot(plot_df,aes(x = pft, y = fates_mortality_rxfire))
gg_now = gg_now + facet_wrap(treatment~site, scales="free",ncol=3)
gg_now = gg_now + geom_bar(stat = "identity")
gg_now = gg_now + scale_x_discrete(labels = pft_keys)
gg_now = gg_now + ylab(expression("Cumulative annual Rx fire mortality (%"~year^-1~")"))
...
for (d in sequence(ndevice)){
    h_output = paste0("Rxfire-mort.",gg_device[d])
