...
main_path   = file.path("/Volumes/Ldian/FATES/sites3-resprt-exp/across-sites-comp")
util_path   = file.path(home_path,"Util/RUtils")
hlm_files   = c("treatment-bldgt_hlm1d.csv",
                "treatment-tea_hlm1d.csv",
                "treatment-stef_hlm1d.csv")
hlm_paths   = file.path(main_path,hlm_files)
pft_files  = c("treatment-bldgt_bypft-dbh15.csv",
               "treatment-tea_bypft-dbh15.csv",
               "treatment-stef_bypft-dbh15.csv")
pft_paths   = file.path(main_path,pft_files)
dbh_files  = c("treatment-bldgt_bydbh.csv",
               "treatment-tea_bydbh.csv",
               "treatment-stef_bydbh.csv")
dbh_paths   = file.path(main_path,dbh_files)
fuel_files = c("treatment-bldgt_byfuel.csv",
               "treatment-tea_byfuel.csv",
               "treatment-stef_byfuel.csv")
...
## method 1: including growth rate of plants died,
## which will be zero, into the calculation of 
## average growth rate; also calculate annual mean
## first

bypft_zero = bypft                                                                     %>% 
  mutate(fates_ddbh_canopy = ifelse(fates_nplant_canopy==0,    
                                    fates_ddbh_canopy*ha_2_m2*fates_nplant_canopy,fates_ddbh_canopy*ha_2_m2),
         fates_ddbh_ustory = ifelse(fates_nplant_ustory==0,
                                    fates_ddbh_ustory*ha_2_m2*fates_nplant_ustory,fates_ddbh_ustory*ha_2_m2),
         fates_ddbh        = ifelse(fates_nplant==0, 
                                    fates_ddbh*ha_2_m2*fates_nplant,fates_ddbh*ha_2_m2))

bypft_zero = bypft_zero                       %>% 
             dplyr::select(-c(time,month))    %>% 
             group_by(case,desc,year,pft)     %>% 
             summarize_all(mean,na.tm=TRUE)   %>% 
             ungroup()

## method 2: excluding growth rate of plants died
...
ddbh_vars = names(pft_df)[grepl("ddbh",names(pft_df))]
n_ddbh = length(ddbh_vars)
for(v in sequence(n_ddbh)){
  var_now = ddbh_vars[v]
  pft_df[[var_now]] = ddbh_scale_fun(pft_df,var_now)
}

ctrl = pft_df %>% filter(case %in% c(1,5,9))
ctrl = ctrl                    %>% 
       group_by(case,desc)     %>% 
...
for(d in sequence(n_delta)){
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"_PFTSplit-smoothed-15-NA.",gg_device[d])
...
overall_na  = overall_na                     %>% 
 ...
 overall_df = overall_na 

ddbh_vars = names(overall_df)[grepl("ddbh",names(overall_df))]
n_ddbh = length(ddbh_vars)
for(v in sequence(n_ddbh)){
  var_now = ddbh_vars[v]
  overall_df[[var_now]] = ddbh_scale_fun(overall_df,var_now)
}

ctrl_all = overall_df %>% filter(case %in% c(1,5,9))
ctrl_all = ctrl_all                %>% 
           group_by(case,desc)     %>% 
           slice(rep(seq_len(n()),times = nsites)) %>% ungroup()
...
for(d in sequence(n_delta)){
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"_byTreatment-smoothed-15-NA.",gg_device[d])
    dummy    = ggsave( filename = f_output
                    , plot     = plot
                    , device   = gg_device[d]
                    , path     = mean_path
...
bydbh_df = bydbh_zero
ddbh_vars = names(bydbh_df)[grepl("ddbh",names(bydbh_df))]
n_ddbh = length(ddbh_vars)
for(v in sequence(n_ddbh)){
  var_now = ddbh_vars[v]
 bydbh_df[[var_now]] = ddbh_scale_fun(bydbh_df,var_now)
}


ctrl_growth = bydbh_df %>% filter(case %in% c(1,5,9))
