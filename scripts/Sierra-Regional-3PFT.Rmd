...
main_path  = file.path("/Volumes/my-drive/FATES/Regional")

util_path   = file.path(home_path,"Util/RUtils")

cases       = c( "sierra-test_CLM_FATES.clm2.nc")

case_files   = file.path(main_path, cases)
ncase        = length(cases)
case_names   = c(1L)

case_desc    = c( "Sierra-best-param-280")
file_desc    ="sierra-280"
...
time_use     = time_val[1:132]          #only retrieve values for the last 11 years of model time
nstep        = length(time_use)
simu_time    = c(1779:1789)
model_yra    = 1651
...
fates_var  = c( "fates_basalarea","fates_crownarea",	
                ...
                "fates_mortality_hydraulic","fates_mortality_rxfire",
                "fates_mortality_termination",
                "fates_nplant_canopy","fates_nplant_ustory",
                "fates_patcharea","fates_vegc_aboveground","fates_lai",
                "fates_nplant") 
...
for (h in struc_loop){
   ...
   for(p in sequence(npfts)){
     ...
     p_plt = p_plt + theme( legend.position    = if_else(p==3,"right","none") 
  ...
  gg_patch = pft_plot[[1]] + pft_plot[[2]] + pft_plot[[3]] +
             plot_layout(guides="collect",widths = c(1,1,1), nrow=1) 
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(case_now,f_vnam,file_desc,gg_device[d])
