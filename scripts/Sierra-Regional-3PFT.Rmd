---
title:  "Sierra-Regional-3PFT.Rmd"
author: "Xiulin Gao"
email:  "xiulingao@lbl.gov"
date:   "18-Mar-2024"
---
## Introduction
This script is used to process CLM-FATES regional runs in California Sierra Nevada region. 
...
made to do post-processing specifically for 3PFT simulations at the regional scale. 
...
main_path  = file.path("/Volumes/Ldian/FATES/FIA-tests")

util_path   = file.path(home_path,"Util/RUtils")

cases       = c( "FIA-test-sites_2000-2020-transient_CLM_FATES.clm2.nc"
                 )
...
case_desc    = c("FIA-test-sites_2001-2017")
file_desc    ="FIA-test-sites_2001-2017"

names(case_desc) = c("1")

## WRF path to read in the correct Lon and Lat info as current FATES cannot output correct curve linear grids info
wrf_path = file.path("~/Google Drive/My Drive/9km-WRF-1980-2020/1981-01.nc")

## Obs paths
gpp_path = file.path("~/Git-repos/NASA-IDS/data/GPP-WRF-creekfire-260424.csv")
lai_path = file.path("~/Git-repos/NASA-IDS/data/LAI-WRF-creekfire_260424.csv")
...
time_use     = time_val[1:length(time_val)]          #only retrieve values for the last 11 years of model time
nstep        = length(time_use)
simu_time    = c(2001:2017)
model_yra    = 2001
...
## which year to use observation for model benchmark
obs_yr1 = 2001
obs_yr2 = 2017
...
fates_var  = c( "fates_basalarea","fates_crownarea",	
...
szpf_var = c("fates_nplant","fates_basalarea",
             ...
             "fates_mortality_termination","fates_mortality_cstarv",
             "fates_mortality_hydraulic","fates_mortality_freezing","fates_mortality_background")
...
#---~---
  for (p in sequence(nbypft-n_varpft_last)){
      ...
      p1             = rowSums(ncm_dat[,2:13]) # for woody PFT we ignore the first size class
      p2             = rowSums(ncm_dat[,15:26])
      p3             = rowSums(ncm_dat[,28:39])
      p4             = rowSums(ncm_dat[,41:52])
      nc_aggr        = rbind(p1,p2,p3,p4)
...
```{r CWCDataModelComparison}
cwc_obs = raster("~/Google Drive/My Drive/2016_bulk_density_9km.tif")
...
for (d in sequence(nbydbh)){
  ...
  if (is.null(now_dbhmelt)){
      now_dbhmelt = now_melt
        ...
        now_dbhmelt = now_dbhmelt %>% left_join(now_melt,by=c("case", "lon","lat","time","dbh"))
...
pft_melt = NULL
loop_p   = c("fates_basalarea",
             #"fates_crownarea",
             #"fates_canopycrownarea",
             "fates_vegc_aboveground",
             "fates_lai",
             "fates_gpp",
             "fates_mortality_background",
...
for(v in sequence(n_obs)){
    ...
    if(obn %in% c("gpp_ob","lai_ob")){
      ob_ameanAll  = ob_now                                 %>% 
                     filter(year %in% c(obs_yr1:obs_yr2))   %>% 
...
cell_mmean = all                                                    %>% 
             ...
             dplyr::select(-c(lon,lat,year,time))                   %>% 
             summarize_all(mean,na.rm=FALSE)                        %>% 
             ungroup()                                                                   
...
cell_amean =  all                                                        %>% 
              ...
              dplyr::select(-c(lon,lat,month,time))                      %>% 
              summarize_all(mean,na.rm=FALSE)                            %>%
              ungroup()                                                 
 
cell_amean = orig_xy %>% left_join(cell_amean,by=c("lon_f","lat_f"))
 
 ## To look at structure is better looking at the stabilized results
 ## while interested in mortality, probably better looking at
 ## simulations from the earlier model time especially when PFT tend to die off
cellmean_struc  = all                                           %>% 
                  group_by(case,lon_f,lat_f,pft)                %>% 
                  dplyr::select(-c(lon,lat,time,year,month))    %>% 
                  summarize_at(c("fates_vegc_aboveground",
                                 #"fates_crownarea",
                                 #"fates_canopycrownarea",
                                 "fates_basalarea",
                                 "fates_lai",
...
cellmean_mort    = all                                           %>% 
                   group_by(case,lon_f,lat_f,pft)                %>% 
                   dplyr::select(-c(lon,lat,time,year,month))    %>%
                   summarize_at(c("fates_mortality_background",
                                   "fates_mortality_cstarv",
                                   "fates_mortality_freezing",
                                   "fates_mortality_hydraulic",
                                   "fates_mortality_termination"
...
ctrl1  =    all                                 %>% 
           filter(case==2)                     %>% 
           rename_at(all_of(c("fates_basalarea",
                       "fates_crownarea",
                       "fates_canopycrownarea",
                       "fates_vegc_aboveground",
                       "fates_lai",
                       "fates_gpp",
                       "fates_nplant",
                       mortout)),function(x) {paste0(x,"_ctrl")})  %>% 
         dplyr::select(-c(lon,lat,year,month,case))
...
case1 =    all     %>% filter(case==1)           %>%  dplyr::select(-c(lon,lat,year,month,case)) %>% 
                   left_join(ctrl1,by=c("lon_f","lat_f","time","pft"))

case2 =    all     %>% filter(case==3)           %>%  dplyr::select(-c(lon,lat,year,month,case)) %>% 
                   left_join(ctrl2,by=c("lon_f","lat_f","time","pft"))

case2 =    case2               %>% 
           mutate(fates_basalarea = ifelse(is.na(fates_basalarea_ctrl), 
                                          fates_basalarea,
                                          fates_basalarea - fates_basalarea_ctrl),
                  fates_crownarea = ifelse(is.na(fates_crownarea_ctrl),
                                          fates_crownarea,
                                          fates_crownarea - fates_crownarea_ctrl),
                  fates_vegc_aboveground = ifelse(is.na(fates_vegc_aboveground_ctrl),
                                          fates_vegc_aboveground,
                                  fates_vegc_aboveground - fates_vegc_aboveground_ctrl),
                 fates_mortality_fire = ifelse(is.na(fates_mortality_fire_ctrl),
                                           fates_mortality_fire,
                                           fates_mortality_fire - fates_mortality_fire_ctrl),
                 fates_mortality_rxfire= ifelse(is.na(fates_mortality_rxfire_ctrl),
                                          fates_mortality_rxfire,
                                    fates_mortality_rxfire - fates_mortality_rxfire_ctrl)) %>% dplyr::select(pft,lon_f,lat_f,
                  fates_basalarea,fates_crownarea,fates_vegc_aboveground,
                  fates_mortality_fire, fates_mortality_rxfire)

case2 = case2                                  %>%  
        group_by(lon_f,lat_f,pft)              %>% 
        summarize_all(mean,na.rm=FALSE)        %>% 
        ungroup()
           
case2 = orig_xy %>% left_join(case2,by=c("lon_f","lat_f"))         
...
```{r, label="plot-pft-time-series-for-selected-sites"}

cords_zero = hlm1d %>% filter(!is.na(fates_vegc_aboveground)) #& fates_vegc_aboveground >= 10) 
cords = cords_zero %>% dplyr::select(lon,lat) %>% distinct()

pft_sub = bypft %>% filter(lon %in% cords$lon & lat %in% cords$lat)
mortout_pft = names(pft_sub)[grepl("mortality", names(pft_sub))]
...
nsite = length(unique(pft_sub$site))
pft_loop = which(fatesvar_new$vnam %in% names(pft_sub))
pft_legend = "Plant functional types"
 ...
 for (f in pft_loop){
  ...
  f_bypft$site = factor(f_bypft$site,levels=sequence(nsite))
  ...
  if (f_stack){
     ...
     gg_now = gg_now + facet_wrap(.~site,ncol=5)
     ...
     gg_now = gg_now + facet_wrap(.~site,ncol=2)
 ...
 # gg_now = gg_now + labs(title=desc_now)
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(f_vnam,"-tspft-",".",gg_device[d])
...
```{r,label="plot-pre-vs-during-mega-droughts-HydroMort",message=FALSE,results="hide"}

plot_df = bypft %>% filter(lon %in% cords$lon & lat %in% cords$lat)
plot_df = plot_df                       %>% 
          group_by(lon,lat)             %>% 
          mutate(site = cur_group_id()) %>% 
          ungroup()
plot_df = plot_df                       %>% 
          ...
          dplyr::select(site,year,pft,
                        fates_nplant,
                        fates_mortality_hydraulic) %>% 
          group_by(site,year,pft)       %>%  
          summarize_all(mean,na.rm=TRUE) %>% 
          ungroup()                     %>% 
          mutate_at(c("fates_mortality_hydraulic"),
                      ~ find_mort(x=.x,
                        n= ifelse(.x=="fates_mortality_ustory", 
                                 .data$fates_nplant_ustory, 
                                 .data$fates_nplant)))
...
plot_df = plot_df                         %>% 
          ...
          mutate(yr_group = cut(year,breaks=c(-Inf,2011,Inf),
                              labels=c("2007-2011","2012-2016"))) %>% 
          group_by(site,pft,yr_group)     %>% 
          summarize_all(sum,na.rm=TRUE)   %>% 
          ungroup()                       %>% 
          dplyr::select(-fates_nplant)    

plot_df = plot_df %>% mutate(pft=case_when(pft=="1" ~ "Pine",
                                           pft=="2" ~ "Cedar",
                                           pft=="3" ~ "Fir",
                                           pft=="4" ~ "Oak"))

pft_loop = which(fatesvar_new$vnam %in% names(plot_df))
 ...
 for (f in pft_loop){
  ...
  gg_now = ggplot(data=f_bypft,aes_string(x="pft",y=f_vnam,fill="yr_group"))
  gg_now = gg_now + facet_wrap(.~site,ncol=5)
  gg_now = gg_now + scale_fill_manual(values=c("2007-2011" = "#18B0D6",
                                                 "2012-2016" = "#D68D18"))
  gg_now = gg_now + geom_bar(position=position_dodge(),stat="identity",width=1) 
 # gg_now = gg_now + labs(title=desc_now)
  gg_now = gg_now + scale_x_discrete(limits=c("Pine","Cedar","Fir","Oak"))
  gg_now = gg_now + xlab("")
  gg_now = gg_now + ylab(desc.unit(desc=f_desc,unit=untab[[f_unit]],twolines=TRUE))
  gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size = 0.5,base_rect_size =0.5)
  gg_now = gg_now + theme( legend.position   = "bottom"
  ...
  # Save plot in every format requested.
  for (d in sequence(ndevice)){
    f_output = paste0("PFT-drough-mort-timewindow",".",gg_device[d])
    dummy = ggsave( filename = f_output
                  , plot     = gg_now
                  , device   = gg_device[d]
                  , path     = tspft_path
...
```{r,label="plot-percent-fir-across-climate-space",message=FALSE,results="hide"}
...
plot_df = plot_df                       %>% 
          ...
          ungroup()                        %>% 
          group_by(site,year)              %>% 
          mutate(across(starts_with("fates_"),
                      ~ sum(.x, na.rm=TRUE),
                      .names="sum_{.col}")) %>% 
          ungroup()
...
plot_df = plot_df                                   %>% 
          mutate(across(starts_with("fates_"),
          ~ .x / get(paste0("sum_",cur_column())) * 100)) %>% 
          group_by(site,pft)                        %>% 
          dplyr::select(-year)                      %>% 
          summarize_all(mean,na.rm=T)               %>% 
          ungroup()
  
clim = hlm1d %>% filter(!is.na(fates_gpp)) 
clim = clim                          %>% 
       group_by(lon,lat)             %>%   
       mutate(site = cur_group_id(),
              rain = rain + snow)    %>% 
...
clim = clim                          %>% 
       ...
       summarize_all(mean,na.rm=T)    %>% 
       ungroup()
plot_df = plot_df %>% left_join(clim,by=c("site"))       
plot_df = plot_df %>% mutate(pft=case_when(pft=="1" ~ "Pine",
                                           pft=="2" ~ "Cedar",
                                           pft=="3" ~ "Fir",
                                           pft=="4" ~ "Oak"))
...
targ_pft = "Fir"
 for (f in pft_loop){
  ...
  #f_stack  = FALSE
  cat0("   - ",f_desc,".")
  f_desc_low = tolower(f_desc)

  # Set plotting characteristics.
  f_bypft = plot_df %>% filter(pft==targ_pft)

  gg_now = ggplot(data=f_bypft,aes_string(x="sum_precip",y="mean_temp",z=f_vnam))
  gg_now = gg_now + stat_summary_2d(fun = mean, bins = 30)
  gg_now = gg_now + scale_fill_viridis_c(labels = scales::percent_format(scale = 1))
  gg_now = gg_now + labs(title=paste("Percent",tolower(targ_pft),f_desc_low, "across climate space",sep=" "),
                         x="Precipitation (mm)",
                         y=expression("Temperature"~"("~degree~C~")"),
                         fill = paste(f_desc,"(%)",sep=" "))
  gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size = 0.5,base_rect_size =0.5)
  ...
  # Save plot in every format requested.
  for (d in sequence(ndevice)){
    f_output = paste0(targ_pft,"-",f_desc_low,".",gg_device[d])
    dummy = ggsave( filename = f_output
                  , plot     = gg_now
                  , device   = gg_device[d]
                  , path     = tspft_path
...
```{r,label="plot-size-structure",message=FALSE,results="hide"}

plot_df = bydbh %>% filter(lon %in% cords$lon & lat %in% cords$lat)
plot_df = plot_df                       %>% 
          group_by(lon,lat)             %>% 
          mutate(site = cur_group_id()) %>% 
          ungroup()
...
plot_df = plot_df                          %>% 
          filter(year %in% c(20013:2017))  %>% 
          group_by(site,dbh)               %>% 
          summarize_all(mean,na.rm=TRUE)   %>% 
          ungroup()                        

pft_loop = which(fatesvar_new$vnam %in% names(plot_df))
dbh_labs = dbhinfo$labs[2:13]
dbh_labs = parse(text=dbh_labs)
names(dbh_labs) = dbhinfo$id[2:13]
 ...
 for (f in pft_loop){
  ...
  # Set plotting characteristics.
  f_bydbh = plot_df %>% filter(!dbh %in% c(1))
  f_bydbh$site = factor(f_bydbh$site,levels=sequence(nsite))
  f_bydbh$dbh  = factor(f_bydbh$dbh,levels=unique(f_bydbh$dbh))

  gg_now = ggplot(data=f_bydbh,aes_string(x="dbh",y=f_vnam))
  gg_now = gg_now + facet_wrap(.~site,ncol=5,scale="free_y")
  ...
  # Save plot in every format requested.
  for (d in sequence(ndevice)){
    f_output = paste0("size-structure",".",gg_device[d])
    dummy = ggsave( filename = f_output
                  , plot     = gg_now
                  , device   = gg_device[d]
                  , path     = tspft_path
...
```{r,label="plot-fuel-time-siries",message=FALSE,results="hide"}

plot_df = byfuel                              %>% 
          filter(!is.na(fates_fuel_amount))   %>% 
          group_by(lon,lat)                   %>% 
          mutate(site = cur_group_id())       %>% 
          ungroup()
...
plot_df = plot_df                             %>% 
          ...
          dplyr::select(-fuel)                %>%
          group_by(site,time)                 %>% 
          summarize(fates_fuel_amount=sum(
            fates_fuel_amount,na.rm=TRUE
          ))                                  %>% 
          ungroup()                           %>% 
          mutate(year=year(time))             %>% 
          group_by(site,year)                 %>% 
          summarize(fates_fuel_amount=
                      mean(fates_fuel_amount)) %>% 
          ungroup()                           %>% 
          mutate(year=make_date(year))

sfuelts_plot = ggplot(plot_df, aes(x=year,y=fates_fuel_amount)) +
  geom_line(lwd=0.5) +
  facet_wrap(.~site,ncol=5,scale="free_y")  +
  labs(x="Year",
       y=expression("Total burnable fuel amount"~"("~kg~C~m^-2~")"))
ggsave(sfuelts_plot,path=plot_main,filename="sumfueload-time.jpeg",width=gg_widthn*0.7,height=gg_height*0.6,dpi=gg_depth) 
...
for (h in struc_loop){
   ...
   h_legend =  1
   cat0("   - ",f_desc,".")
   var_now  = mmean_now %>% dplyr::select(all_of(c("lon","lat","pft",f_vnam)))
   
   y         = unlist(var_now %>% dplyr::select(f_vnam))
   ...
   for(p in sequence(npfts)){
     pft_now  = p
     df       = var_now %>% filter(pft==pft_now)   
     fil_p    = matrix(df[[f_vnam]], nrow=147,ncol=151)
     x_arr    = matrix(df$lon,nrow=147,ncol=151)
     y_arr    = matrix(df$lat,nrow=147,ncol=151)
     p_star   = st_as_stars(fil_p)
     p_star   = st_as_stars(p_star, curvilinear=list(X1=x_arr,X2=y_arr), crs=wgs)
     p_sf     = st_as_sf(p_star,as_points=FALSE,na.rm=FALSE)
     wd_ratio = tmaptools::get_asp_ratio(p_sf)
   
     p_plt  = ggplot()
     p_plt  = p_plt + geom_sf(data=p_sf,aes(fill=A1),color=NA)
     p_plt  = p_plt + coord_sf(crs=st_crs(wgs)) 
     p_plt  = p_plt + scale_fill_gradientn(colours=cols, guide="colorbar", na.value="white", 
                                             values=brks, limits=rng,oob = scales::squish) 
     p_plt = p_plt + geom_sf(data = ca_co, color = alpha("black", alpha=1),lwd=0.1,fill=NA) 
     p_plt = p_plt + scale_x_continuous(limits = c(-124.8, -114.0),expand = c(0,0))
     p_plt = p_plt + scale_y_continuous(limits = c(32, 42.2),expand = c(0,0))
     p_plt = p_plt + labs(title=pftinfo$short[p]
                         , x=if_else(p %in% c(3,4),"Longitude","")
                         , y="Latitude"
                         , fill=desc.unit(desc=f_desc,unit=untab[[f_unit]],twolines =TRUE))
     p_plt = p_plt + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
     p_plt = p_plt + theme( legend.position    = if_else(p==4,"right","none") 
  ...
  gg_patch = pft_plot[[1]] + pft_plot[[2]] + pft_plot[[3]] + pft_plot[[4]]
             plot_layout(guides="keep",nrow=1) 
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(case_now,f_vnam,"abs.",gg_device[d])
...
hlm_var   = c("fates_gpp","elai","fates_vegc_aboveground")
fire_var  = c("fates_burnfrac","fates_fire_intensity")
hlm1d       = hlm1d                       %>% 
              mutate( lon = round(lon,5)
                    , lat = round(lat,5))
...
bf_mod   =  hlm1d                                                                  %>% 
            dplyr::select(all_of(c("case","time","lon","lat",
                                   ...
                                   "fates_rxfire_burnfrac","fates_rx_burn_window",
                                   "fates_fire_intensity_burnfrac",
                                   "fates_rxfire_intensity_burnfrac",
                                   "fates_fuel_amount","rain","snow",
                                   "q2m","qaf","taf","tsa","u10")))                %>% 
            mutate(month=month(time))                                              %>% 
...
mean_bf =   bf_mod                                     %>%
            mutate(fates_fire_intensity_burnfrac = ifelse(is.na(fates_burnfrac) | fates_burnfrac == 0,    fates_fire_intensity_burnfrac, 
                                                           fates_fire_intensity_burnfrac / fates_burnfrac),
                   fates_rxfire_intensity_burnfrac = ifelse(is.na(fates_rxfire_burnfrac) | fates_rxfire_burnfrac == 0, 
                                                            fates_rxfire_intensity_burnfrac,
                                                             fates_rxfire_intensity_burnfrac / fates_rxfire_burnfrac)) %>% 
            group_by(case,lon_f,lat_f)                 %>%
            dplyr::select(-c(time,lon,lat))            %>% 
            summarize_all(mean,na.rm=FALSE)            %>% 
            ungroup()   
...
mean_bfall = hlm1d                                          %>% 
             dplyr::select(all_of(c("case","lon_f","lat_f",
                                    fire_var)))             %>% 
            mutate(fates_fire_intensity_burnfrac = ifelse((is.na(fates_burnfrac) | fates_burnfrac == 0), 
                                                           fates_fire_intensity_burnfrac, 
                                                           fates_fire_intensity_burnfrac / fates_burnfrac),
                   fates_rxfire_intensity_burnfrac = ifelse((is.na(fates_rxfire_burnfrac) | fates_rxfire_burnfrac == 0), 
                                                             fates_rxfire_intensity_burnfrac,
...
allmean_hlm = hlm1d                                      %>%
              dplyr::select(all_of(c("case","lon_f",
                                     "lat_f",hlm_var)))  %>% 
              #mutate(rain_tot = rain + snow)             %>% 
              group_by(case,lon_f,lat_f)                 %>% 
              summarize_all(mean,na.rm=FALSE)            %>% 
              ungroup()
...
```{r, label="hlm-relative-difference"}
hlm_dif = c("fates_vegc_aboveground",
            "fates_fuel_amount",
            "fates_gpp",
            "elai",
            "fates_burnfrac",
...
ctrl1_hlm = hlm1d                                         %>% 
            filter(case==4)                               %>% 
            dplyr::select(all_of(c("time","lon_f",
                                   "lat_f",hlm_dif)))     %>% 
            rename_at(vars(hlm_dif),function(x) {paste0(x,"_ctrl")})
case1_hlm = hlm1d                                         %>% 
            filter(case==3)                               %>% 
            dplyr::select(all_of(c("time","lon_f",
                                   "lat_f",hlm_dif)))     %>% 
            left_join(ctrl1_hlm, by=c("lon_f","lat_f","time"))

case1_hlm = case1_hlm                                     %>% 
            mutate(fates_vegc_aboveground = ifelse(!is.na(fates_vegc_aboveground_ctrl), 
                                                      fates_vegc_aboveground-fates_vegc_aboveground_ctrl, NA),
                      fates_fuel_amount = ifelse(!is.na(fates_fuel_amount_ctrl),
                                                 fates_fuel_amount-fates_fuel_amount_ctrl, NA),
                      fates_gpp = ifelse(!is.na(fates_gpp_ctrl),
                                         fates_gpp-fates_gpp_ctrl,NA),
                      elai = ifelse(!is.na(elai_ctrl),
                                    elai-elai_ctrl,NA),
                      fates_burnfrac = ifelse(!is.na(fates_burnfrac_ctrl),
                                              fates_burnfrac-fates_burnfrac_ctrl,NA),
                      fates_fire_intensity_burnfrac = ifelse(!is.na(fates_fire_intensity_burnfrac_ctrl),
                                    fates_fire_intensity_burnfrac-fates_fire_intensity_burnfrac_ctrl,NA)) %>% 
            group_by(lon_f,lat_f)           %>% 
            dplyr::select(-c(time,fates_vegc_aboveground_ctrl,fates_fuel_amount_ctrl,
                             fates_gpp_ctrl,elai_ctrl,fates_burnfrac_ctrl,
                             fates_fire_intensity_burnfrac_ctrl)) %>% 
            summarize_all(mean, na.rm=FALSE)
        

  diff_hlm  = orig_xy %>% filter(case==1) %>% dplyr::select(-case) %>% left_join(case1_hlm,by=c("lon_f","lat_f")) %>% 
...
for(c in sequence(ncase)){
  case_now = case_names[c]
  desc_now = case_desc[c]
mmean_now        = mean_pseason %>% filter(case == case_now) 
for (h in emean_loop){
   h_vnam   = hlm1dvar$vnam[h] 
   ...
   h_legend =  1
   cat0("   - ",h_desc,".")
   all_df   = mmean_now %>% dplyr::select(all_of(c("lon","lat",h_vnam)))
   
#   for(m in sequence(12)){
#   mon_now = m 
   var_df = all_df #%>% filter(month==mon_now)
   ...
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(case_now,h_vnam,"peak.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot     = gg_now
                    , device   = gg_device[d]
                    , path     = secy_path
...
```{r,label="plot-total-precipitation"}

for(c in sequence(ncase)){
  case_now = case_names[c]
  desc_now = case_desc[c]
  mmean_now = mean_allseson %>% filter(case == case_now) 
   var_df   = mmean_now %>% dplyr::select(all_of(c("lon","lat","rain_tot")))
   fil_var  = matrix(var_df$rain_tot, nrow=147,ncol=151)
   x_arr    = matrix(var_df$lon,nrow=147,ncol=151)
   y_arr    = matrix(var_df$lat,nrow=147,ncol=151)
   wrf_star = st_as_stars(fil_var)
   wrf_star = st_as_stars(wrf_star, curvilinear=list(X1=x_arr,X2=y_arr), crs=wgs)
   ...
   gg_now = gg_now + labs(title=desc_now
                         , x="Longitude"
                         , y="Latitude"
                         , fill=expression("Total precipitation"~"("~mm~month^-1~")"))
   gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
   gg_now = gg_now + theme( legend.position   = "right" 
                          , panel.background  = element_blank()
                          , panel.border      = element_rect(linewidth = 1.6, fill=NA)
   ...
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(case_now,"totrain-Transallmean.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot     = gg_now
                    , device   = gg_device[d]
                    , path     = secy_path
...
szmort$lon_f   = factor(szmort$lon,levels=unique(szmort$lon))  
szmort$lat_f   = factor(szmort$lat,levels=unique(szmort$lat))
orig_xy        = szmort %>% dplyr::select(case,lon,lat,lon_f,lat_f,dbh) %>% distinct()

szmort     =      szmort                           %>% 
                  group_by(case,lon_f,lat_f, dbh)  %>% 
                  summarize_at(mortout_sz, mean, na.rm=FALSE)
szmort     = orig_xy %>% left_join(szmort, by=c("case","lon_f","lat_f","dbh")) %>% dplyr::select(-c(lon_f,lat_f))
emean_loop = which(fatesvar_szpf$vnam %in% mortout_sz)
gg_emean   = list()
for(c in sequence(ncase)){
  case_now = case_names[c]
  desc_now = case_desc[c]
  mmean_now        = szmort %>% filter(case == case_now) 
for (h in emean_loop){
   h_vnam   = fatesvar_szpf$vnam[h] 
   h_desc   = fatesvar_szpf$desc[h]
   h_unit   = fatesvar_szpf$unit[h]
   h_vqlwr  = paste0(h_vnam,"_qlwr")
   h_vqupr  = paste0(h_vnam,"_qupr")
   h_legend =  1
   cat0("   - ",h_desc,".")
   var_df   = mmean_now %>% dplyr::select(all_of(c("lon","lat","dbh",h_vnam)))
   for(d in sequence(ndbhs)){
     dbh_now = var_df %>% filter(dbh == d)
     fil_var  = matrix(dbh_now[[h_vnam]], nrow=147,ncol=151)
     x_arr    = matrix(dbh_now$lon,nrow=147,ncol=151)
     y_arr    = matrix(dbh_now$lat,nrow=147,ncol=151)
     wrf_star = st_as_stars(fil_var)
     wrf_star = st_as_stars(wrf_star, curvilinear=list(X1=x_arr,X2=y_arr), crs=wgs)
     wrf_sf   = st_as_sf(wrf_star,as_points=FALSE,na.rm=FALSE)
     ...
     gg_now = gg_now + scale_x_continuous(limits = c(-124.8, -114.0),expand = c(0,0))
     gg_now = gg_now + scale_y_continuous(limits = c(32, 42.2),expand = c(0,0))
     gg_now = gg_now + labs(title=paste0(desc_now, "-Size",d)
                           , x="Longitude"
                           , y="Latitude"
                           , fill=desc.unit(desc=h_desc,unit=untab[[h_unit]],twolines =TRUE))
     gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
     gg_now = gg_now + theme( legend.position   = "right" 
                            , panel.background  = element_blank()
                            , panel.border      = element_rect(linewidth = 1.6, fill=NA)
   ...
   # Save plots.
   for (v in sequence(ndevice)){
    h_output = paste0(case_now,h_vnam,d,".",gg_device[v])
    dummy    = ggsave( filename = h_output
                    , plot     = gg_now
                    , device   = gg_device[v]
                    , path     = secy_path
...
#      ) #end theme
  ...
  #p = wrap_plots(mod,obs,scat) + 
   ...
   ggsave(p2, filename=fname,path=secy_path,width=gg_widthn,height=gg_height,dpi=gg_depth,device=gg_device[1]) 
